...
| Doc-Invariant Registry | `docs/doc-invariant-registry.md` | The permanent-vs-cycle-scoped guard-lifecycle rule, the registry schema/runner contract (including the `section_kind` anchor kinds and the `--self-test` mutation-teeth mode), or a retired-guard disposition table changes (issues #951, #76, #85, #119) | Orchestrator AI |
...
| Scope-Fit Verdict Inheritance | `scripts/test/suite-coverage.sh` + `scripts/test/green-tree-store.sh` + `scripts/test/green-tree-register.sh` + `docs/adr/0019-scope-fit-verification-policy.md` | The resolver's resolution order (out-of-tree declaration, dirty worktree, whole-tree fast path, shared tree match, coverage fold, input-hash short-circuit, reach test, non-candidate record), its candidate-set source — the `--include-worktree` selection call that makes the candidate set the committed delta unioned with the uncommitted worktree, and the reach call that deliberately omits the flag — its stdout/stderr contract or exit codes, the declared per-suite reason vocabulary — the `reason-tokens` block in the resolver's header, which is that vocabulary's one normative home, every other passage carrying these tokens being derived from it and held to it by `tests/test-suite-coverage-agreement.sh` — the BLOCK-degrades-to-executing fallback, the `--candidates selection|all` modes, the entry-admission value gate that decides which parsed green-tree entries reach the selectable arrays (`worktree` admits only `clean`; any other value is a malformed entry and BLOCKs), the ledger-derived head validation, the `# out-of-tree-inputs:` header field or its presence obligation, the phase-step idiom's exit-status guard, the `suites` token grammar `<path>[@<input-hash>]` and the input closure it hashes (the suite's own path, its `ci-subject` matches, and every path under `tests/lib/**`), the declared citation-basis vocabulary — the `citation-basis` block in the resolver's header, that vocabulary's one normative home — the shared store's location under `$AUTOFLOW_ARCHIVE_ROOT/<repo-key>/green-trees/`, its cache dispositions (prunable; a malformed entry skipped with one warning rather than a BLOCK; no gate reads it), or the writer's surface (`--append` / `--match [--cover-enumerated]` / `--store-path` / `--prune [--keep <n>]`, its capture-point re-take and drift refusal, and its default retention) changes (issues #112, #130) | Orchestrator AI |
