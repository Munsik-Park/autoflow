...
| Suite Governance | `scripts/test/suite-manifest.sh` + `scripts/test/check-suite-manifest.sh` + `scripts/test/check-suite-leaf.sh` + `tests/lib/harness-pins.sh` | The suite header grammar (`ci-subject` / `lane` / `retire-with` / `cycle-arm` / `budget-secs`), the subject-set enumeration or its exclusions, `SUITE_BUDGET_CEILING_SECS`, the leaf rule's closed shape table, or the shared harness ok-count pin changes (issue #103) | Orchestrator AI |
| Suite Selection and Execution | `scripts/test/select-suites.sh` + `scripts/test/run-suites.sh` + `scripts/test/check-step-reconciliation.sh` | The selection predicate, the push / empty-delta full-set rule, the fail-loud base contract, the runner's de-duplication or `timeout`-based budget enforcement, the CI `select` step's output shape, or the sentinel-delimited `if:` guard / step-`id` convention the reconciliation reads changes (issue #103) | Orchestrator AI |
