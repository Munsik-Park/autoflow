...
| Doc-Invariant Registry | `docs/doc-invariant-registry.md` | The permanent-vs-cycle-scoped guard-lifecycle rule, the registry schema/runner contract (including the `section_kind` anchor kinds and the `--self-test` mutation-teeth mode), or a retired-guard disposition table changes (issues #951, #76) | Orchestrator AI |
