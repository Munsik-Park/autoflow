...
| CI-Green Confirm Script | `scripts/handoff/confirm-ci-green.sh` (+ `tests/test-issue-25-confirm-ci-green.sh`) | The mergeable-precheck / early-block / finite-poll / exit-code contract (`0`/`10`/`11`/`12`/`13`/`64`) or the `CI_POLL_TIMEOUT_SECS`/`CI_POLL_INTERVAL_SECS` tunables change (issue #25) | Orchestrator AI |
