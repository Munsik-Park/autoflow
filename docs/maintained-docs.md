...
| Artifact | Path | Update When | Maintainer |
|----------|------|-------------|------------|
| Issue Proposal Contract | `docs/issue-proposal.md` + `scripts/issue/create-issue.sh` (+ `tests/test-issue-96-issue-create-gate.sh`, `tests/test-issue-96-issue-create-wrapper.sh`) | The draft grammar (`## Title` / `## Grounds` / `## Duplicate check` / `## Body`), the `.autoflow/` top-level containment rule, the term-derivation floors (ASCII 4 / non-ASCII 2 codepoints, locale-free), the open-only search scope, the saturated-page refusal, the exit-code contract (`0`/`64`/`65`/`70`), the `issue-<N>-proposal.md` rename, or the hook's `gh issue create` deny changes (issue #96) | Orchestrator AI |
