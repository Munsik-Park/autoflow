...
5. Confirm CI is green on the created PR(s). **[MUST]** Before polling for a green status, read `gh pr view <N> --json mergeable,mergeStateStatus` first. The host CI (`continuous-integration/jenkins/pr-merge`) builds the **merge revision** (the PR merged onto its base), so a `CONFLICTING` / `mergeStateStatus: DIRTY` PR yields no merge commit and Jenkins **silently skips the build** — the status stays 0-count and a naive "wait for green" loop hangs forever. Do **not** misread the empty status as a webhook miss (webhook deliveries are 200 OK in this case — see #570). On `CONFLICTING`, do **not** wait for CI; branch by cause — a concurrent cycle advancing `main`'s `services` gitlink → resolve via [`external-review-sequencing.md`](external-review-sequencing.md) > Reconcile preflight; any other merge conflict → resolve against `origin/main` (rebase / merge) and re-push (HANDOFF internal retry). Poll for green **only** once the PR reads `mergeable: MERGEABLE` / `mergeStateStatus: CLEAN`. If no check ever publishes while the PR is `MERGEABLE`, confirm the webhook / scan fallback (the job's `PeriodicFolderTrigger`, or a `synchronize` re-push) before escalating to the operator — never read a clean-but-empty status as green.
