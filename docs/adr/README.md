...
| [0020-acceptance-criterion-authority.md](0020-acceptance-criterion-authority.md) | Accepted, amended by ADR-0022 | Changing an issue's acceptance criteria is the operator's authority: ARCHITECT halts with `AC_CHANGE`, the decision is recorded as an `[ac-decision]` ledger entry, and GATE:PLAN / GATE:QUALITY cap on an uncovered difference. |
...
| [0022-test-necessity-and-three-tier-ac-guard.md](0022-test-necessity-and-three-tier-ac-guard.md) | Accepted | A test exists only when it is needed (required behavior + cost of absence, default `none`); acceptance-criterion reductions pass deliberation → external reviewer → operator, and Reconcile narrows to `dropped` / `unreasoned` (`substituted` retired by issue #160). |
