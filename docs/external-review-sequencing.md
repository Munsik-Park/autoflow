...
Its lifecycle has **three** actions, not two — attach, remove, and **re-attach**: (1) **attach** at PR creation, on every PR of the cycle; (2) **remove** by the reviewer on that PR when its review confirms zero `Critical`/`High`/`Medium` findings; (3) **re-attach** by the reviewer when a *later* review of the same PR confirms a `Critical`/`High`/`Medium` finding while the label is absent — the state a Low-only clean round leaves behind when new code is then pushed. Without the third action the gate is one-way and a genuine Medium+ finding on an already-cleared PR reaches the merge actor unsignalled. Attach and remove are mutually exclusive on any single review (zero findings versus at least one), and authority over the label is directional: the reviewer is the **sole** authority for **removal** (the orchestrator is hook-denied), while **attach** is **reviewer-primary with an orchestrator backstop** — the orchestrator re-attaches at HANDOFF step 6.5 ([`autoflow-guide.md`](autoflow-guide.md)) only when the reviewer's own attach did not land, and an attach made in error is undone by a reviewer re-review, never by the orchestrator. All three actions require the label to exist in the target repo — creation stays operator-owned, per the prerequisites above.
