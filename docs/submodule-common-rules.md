...
- `bash scripts/test/run-suites.sh` — the suites this change requires, selected from each suite's own `# ci-subject:` header against the resolved delta. A `push` event, or an empty delta from a resolved base, selects the full set; an unresolvable base, or an enumerated suite with no usable `# ci-subject:` header, is a visible `BLOCK` and a non-zero exit with nothing executed, never a silent empty selection — the message names `--all` and the header migration (`docs/autoflow-guide.md` > RED > Header contract > *Adopting the contract over existing suites*).
