...
3. **`Minimal implementation` weighs comment content and volume, under a floor.** On a target the item reads the REFINE report's `## Comment check` (Decision 23, item 3) — its `comment-ratio` line and its hits — with the comments in the diff. A comment the rule does not admit is depth the acceptance criteria do not need; and even admitted comments whose amount, absolute and relative to the code they sit on, exceeds what a reader of the changed code needs are judged the same way — qualitatively, naming the comment blocks, with no ratio threshold (Decision 23 found no quantitative basis for one). Either finding may lower the item, never below 7 (PR #287 review round 3, Medium 1: content alone left the volume the issue named unweighed). The REFINE comment check stays Decision 23's single procedure, extended to the comments no pattern finds — a restatement of the code, design discussion, another file's contract, commented-out code — which the Developer AI finds by reading: one check, not two.
