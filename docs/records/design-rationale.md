...
### Decision 25: Scope Is Two Recorded Judgments, Not an Acceptance-Criterion ID; a PASS's Recommendations Are Disposed Of; an Acceptance-Criterion Change Can Be Raised From Any Phase
...
**Decision.** Operator decision (issue #275, 2026-09-22), executed at the operator's instruction as orchestrator work outside an AutoFlow cycle.
...
4. **A PASS's recommendations are disposed of.** After the PASS of every rubric-scored gate — GATE:HYPOTHESIS in both forms, GATE:PLAN, AUDIT and GATE:QUALITY, since the finding-coverage contract binds them all (PR #288 review round 2, Medium 1) — the orchestrator disposes of every recommendation as `fix`, `reject` with a reason, `outside` (not directly related) or `operator`, on the two questions and not on whether its file is already in the diff — the file a directly related fix needs can be the one the diff missed (PR #288 review, Medium 1) — in one ledger entry; the precedent is the evaluator's `defect` / `not a defect — <reason>` disposition of REFINE's out-of-scope observations (issue #135). A `fix` travels the route its class names, as a FAIL re-entry does — at GATE:HYPOTHESIS, back to the DIAGNOSE step that wrote the analysis (round 3, Medium 2) — and ends in the recommending gate's narrowed re-score, except where its route runs none: a GATE:PLAN item carried below the decision layer into DISPATCH is scored with the work it enters, and a target comment's direct commit keeps Decision 24's no-re-score rule, each `fix` line closed by its route's own evidence (round 4, Medium 1–2); a resume re-enters past a gate only once its disposition is closed, the entry reading `none` when there was nothing to dispose of (round 3, Medium 1). An `operator` line is closed by a final-disposition line once the operator answers, and every recommendation whose final disposition is not `fix`, with every separation of a directly related problem, goes to the PR body's `## Scope separations`, so the reviewer judges it (round 2, Medium 3).
5. **An acceptance-criterion change can be raised from any phase.** The authority is unchanged — acceptance-criterion content is the operator's (ADR-0020). The entries widen: besides the ARCHITECT deliberation and VERIFY's design contradiction, a role at GREEN, VERIFY or REFINE raises a needed change in its report, and a recommendation reaches the operator through its `operator` disposition. `[ac-decision]` names the phase the change surfaced in and gains a fourth disposition, `added`; an `added` entry covers nothing in either gate backstop, since the criterion it adds is owed a row and a site. Where the cycle re-enters after such a decision is the orchestrator's recorded judgment, and a return to ARCHITECT on that ground consumes no re-entry budget, being bounded by the operator's decision. A return to ARCHITECT after DISPATCH — this one, a recommendation's design `fix`, and the existing VERIFY design contradiction and `design` re-entries, which had no stated shape — spawns both participants fresh on the same transcript, because the phase boundary ended their lifetime, and the Record's delta names the trigger as its origin (round 2, Medium 2).

**The two points the issue left to the design.**

- *Is a fix after a PASS scored?* Yes. #607's ledger (O49) gave "a commit after the gate sends an unscored tree to the reviewer" as the ground for leaving its recommendations unfixed; the narrowed re-score FAIL re-entries already use (issue #232) removes that ground at the cost of one fresh spawn over the touched items. It is not a FAIL and consumes no FAIL cap, and a failing re-score is an ordinary FAIL. The fix pass runs once per gate per cycle on the orchestrator's authority (Decision 7): a directly related recommendation desirable to fix that the re-score raises goes to the operator, who decides whether another pass runs, so every further pass passes through a human. The bound is never a separation reason — a spent pass answers neither half of question 2, and an earlier draft that admitted it would have delivered a known in-scope defect on the bound alone (PR #288 review, Medium 2).
...
**What was rejected.** A single per-issue scope file every role appends to — a new artifact where principle 2 asks for the report the phase already produces; the common heading makes the records findable instead. Having the evaluator tag each recommendation with a `remedy_class` — the class of a PASS recommendation is a route judgment, the orchestrator's under principle 2, and the evaluator's output would grow for findings that fail nothing. Asking the operator about every inclusion — the operator's authority is acceptance-criterion content, an inclusion changes no criterion, and routing every inclusion to the operator would restore the narrowing default under another name.

**Why the devices are documents and two prompts.** No hook reads a scope judgment or a recommendation disposition: they are principle-2 judgments, not authority rules, and a wrong one is caught by `Minimal implementation`, `Impact scope`, GATE:PLAN's `Scope` and the reviewer. The devices behind the old rule were the documents the roles and the evaluator load, and they change here: `docs/submodule-common-rules.md` (Change Surface Rules, GATE:QUALITY linkage), `docs/autoflow-guide.md` (ARCHITECT's artifacts and report routing, GATE:PLAN, GREEN, VERIFY step 3, REFINE, AUDIT, GATE:QUALITY, HANDOFF step 4), `docs/phases/analysis.md` (step 6), `docs/role-contracts.md`, `docs/evaluation-system.md`, `docs/pr-body-guide.md`, `CLAUDE.md` (the `[ac-decision]` grammar, Flow Control, Regressions, the `phase` field) and the implementer, tester and planner agent definitions. Two prompts that execute change with them: the ARCHITECT topic (`scripts/architect/relay-state.sh`) asks the scope question, and the Record scribe (`.claude/workflows/architect-deliberation.js`) writes the `## Scope` section and its `—` rows, its delta heading naming each origin a return to ARCHITECT can have; `test/workflows/run.mjs` and `tests/test-issue-179-relay-state.sh` assert the new sentences. `scripts/ledger/ledger-entry-id.sh` checks identifiers, not dispositions, and `scripts/metrics/cycle-metrics.py` counts `[ac-decision]` headings by their marker, so neither changes.
