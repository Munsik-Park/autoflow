...
2. **The same question is asked at three points**: where the scope is set — DIAGNOSE's task decomposition and the ARCHITECT feature design's `## Scope` section, whose inclusions get verification-design rows with `Issue AC` `—`; during the work — GREEN, VERIFY step 3 and REFINE; and over the gates' recommendations (item 4).
...
4. **A PASS's recommendations take the procedure a reviewer finding takes.** A gate's non-blocking finding and a reviewer's finding are the same kind of thing — a defect named at a location — and only the reviewer's had a procedure: a severity, a `remedy_class` answering whether clearing it moves a decision the deliberation settled, `scripts/gate/remedy-route.sh` for the entry point, four pause criteria, the orchestrator's judgment on `Low`, a ledger record and an attempt cap (HANDOFF step 6.5; Decisions 15 and 17). After the PASS of every rubric-scored gate — GATE:HYPOTHESIS in both forms, GATE:PLAN, AUDIT and GATE:QUALITY, since the finding-coverage contract binds them all (PR #288 review round 2, Medium 1) — the recommendations take that procedure. The one addition is the evaluator's output contract: each recommendation names its subject, its item, its severity and, at `Medium` or above, its `remedy_class` — the classification step 6.5's ingesting subagent performs on a review comment, made here by the evaluator, which is already the classifying authority for a FAIL (Decision 11). Two parts are the gate's own: the fix is verified by the recommending gate's narrowed re-score (issue #232) instead of a reviewer re-review, a re-score that fails being an ordinary FAIL; and where the fix is made follows from the gate's place in the cycle — an entry point the cycle has passed is re-entered as a FAIL's is, a GATE:PLAN recommendation below the decision layer is carried into DISPATCH because no change exists yet and the fix changes no document GATE:PLAN reads (issue #192's layer split), and a GATE:HYPOTHESIS recommendation on the analysis is fixed in DIAGNOSE, where that gate's FAIL goes, so the hypothesis rubric re-scores it (PR #288 review round 3, Medium 2). The two questions of item 1 work inside the procedure: they decide which recommendation is not this issue's work — a separate issue is its follow-up path — and ground the `Low` judgment. Attempts are recorded under a marker of their own, `[gate-autofix]`, so the `[review-autofix]` count step 6.5's cap reads, and the metrics that read the same marker, keep counting reviewer findings only.
5. **An acceptance-criterion change can be raised from any phase.** The authority is unchanged — acceptance-criterion content is the operator's (ADR-0020). The entries widen: besides the ARCHITECT deliberation and VERIFY's design contradiction, a role at GREEN, VERIFY or REFINE raises a needed change in its report, and a gate's PASS recommendation whose fix needs such a change reaches the operator through the procedure's first pause criterion (item 4). `[ac-decision]` names the phase the change surfaced in and gains a fourth disposition, `added`; an `added` entry covers nothing in either gate backstop, since the criterion it adds is owed a row and a site. Where the cycle re-enters after such a decision is the orchestrator's recorded judgment, and a return to ARCHITECT on that ground consumes no re-entry budget, being bounded by the operator's decision. A return to ARCHITECT after DISPATCH — this one, an AUDIT or GATE:QUALITY recommendation of class `design`, and the existing VERIFY design contradiction and `design` re-entries, which had no stated shape — spawns both participants fresh on the same transcript, because the phase boundary ended their lifetime, and the Record's delta names the trigger as its origin (round 2, Medium 2).

**The points the issue left to the design.**

- *Does a `Medium` recommendation hold the transition?* Yes, as a reviewer `Medium` holds the hand-off: the PASS stands as the gate's verdict, and the transition waits until the recommendation is fixed and re-scored, carried to the phase that fixes it, recorded as not this issue's work, or decided by the operator. No hook enforces the wait — a recommendation skipped past it reaches the reviewer, who catches it (principle 3).
- *What bounds the attempts?* Step 6.5's cap, 7, counted over `[gate-autofix]` entries; a re-score that fails counts toward the gate's FAIL cap. Every attempt therefore passes a bound (Decision 7), and the bound is never a separation reason — a spent budget answers neither half of item 1's second question (PR #288 review, Medium 2).
- *How does a resume recognise an unfinished fix?* It does not need to, and no device is added (operator decision, 2026-09-22). A session that ends mid-fix resumes after the last passed gate by the existing Resume procedure; a recommendation left unfixed is caught by the reviewer and resolved by the same procedure at HANDOFF. That is the cost the issue started from, confined to an abnormal end, and PR #288's resume condition — built to prevent it — drew two review findings of its own (rounds 3 and 5).
...
**What was rejected.** A separate disposition scheme for the recommendations — PR #288's `fix` / `reject` / `outside` / `operator` vocabulary, a ledger entry per pass, closure evidence per route, an added Resume condition, one fix pass per gate per cycle and a `## Scope separations` PR-body section. Each piece re-solved a problem step 6.5 had already solved, and its five review rounds found eleven `Medium` findings, every one after the first at a junction with an existing device — resume, re-score, the exception routes, the PR body, the participants' lifetime. A single per-issue scope file every role appends to — a new artifact where principle 2 asks for the report the phase already produces; the common heading makes the records findable instead. Asking the operator about every inclusion — the operator's authority is acceptance-criterion content, an inclusion changes no criterion, and routing every inclusion to the operator would restore the narrowing default under another name.

**Why the devices are documents and prompts.** No hook reads a scope judgment or a recommendation's classification: they are principle-2 judgments, not authority rules, and a wrong one is caught by `Minimal implementation`, `Impact scope`, GATE:PLAN's `Scope`, the recommending gate's re-score and the reviewer. The devices behind the old rule were the documents the roles and the evaluator load, and they change here: `docs/submodule-common-rules.md` (Change Surface Rules, GATE:QUALITY linkage), `docs/autoflow-guide.md` (ARCHITECT's artifacts, report routing and re-discussion, the four gates' PASS lines and GATE:QUALITY > *PASS recommendations*, GREEN, VERIFY step 3, REFINE), `docs/phases/analysis.md` (step 6 and the loop check's gate call site), `docs/role-contracts.md` and `docs/evaluation-system.md` (the recommendation contract), `CLAUDE.md` (the `[ac-decision]` grammar, the `[gate-autofix]` marker, Flow Control, Regressions, the `phase` field) and the implementer, tester, planner, evaluator and loop-check agent definitions. Two prompts that execute change with them: the ARCHITECT topic (`scripts/architect/relay-state.sh`) asks the scope question, and the Record scribe (`.claude/workflows/architect-deliberation.js`) writes the `## Scope` section and its `—` rows, its delta heading naming each origin a return to ARCHITECT can have; `test/workflows/run.mjs` and `tests/test-issue-179-relay-state.sh` assert the new sentences. `scripts/gate/remedy-route.sh` is called unchanged — its mapping is the class set's, whoever classified it; `scripts/ledger/ledger-entry-id.sh` checks identifiers, not markers; and `scripts/metrics/cycle-metrics.py` counts `[review-autofix]` and `[ac-decision]` headings by their marker, which the separate `[gate-autofix]` marker leaves as they were.

**Route.** Operator decision recorded here per [`development-guideline.md`](../development-guideline.md) > ADR Policy, which accepts "an ADR or a documented owner decision". It touches a trigger area — evaluation policy: the evaluator's output contract and the `Minimal implementation`, `Impact scope` and GATE:PLAN `Scope` criteria — and is recorded as the documented owner decision, on the precedent of Decisions 11 and 24. It widens ADR-0020's entry points and disposition vocabulary without changing its authority principle; ADR-0020 carries an amendment note.
