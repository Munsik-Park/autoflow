...
3. **The REFINE comment check is a signal, not a gate.** The check runs in REFINE step 1 over the lines the cycle's diff adds, disposes of each pattern hit, and records the comment-line ratio as an observation. It is not a hook denial: a rule binds authority or prevents self-certification (`CLAUDE.md` > Rule Scope, principle 1), and a comment's content does neither; identifying a comment line needs the file's language, and a hook that parsed every target's languages would encode per-target tooling in the framework, which Decision 14 rejects ("encoding one repository's tool … in the framework would be wrong for every other target"); and #276, which settles how a comment divergence is handled, has it block neither a gate nor a PR. The ratio has no threshold because no quantitative basis for a right comment density was found; the evidence gathered in issue #284 bears on the kind of comment, not the amount.
