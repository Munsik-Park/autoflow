...
| [0022-test-necessity-and-three-tier-ac-guard.md](0022-test-necessity-and-three-tier-ac-guard.md) | Accepted; Reconcile tier-3 trigger superseded by issue #166; `delivery-check`'s definition amended by ADR-0024; decision 1's retention role replaced by ADR-0024 D1 (issue #222); decision 6 and tier 3 amended by issue #275 | A test exists only when it is needed (required behavior + cost of absence, default `none`); acceptance-criterion reductions pass deliberation → external reviewer → operator, and tier 3 is reached through the orchestrator's routing of the deliberation report and the two gates' AC-authority checks. |
