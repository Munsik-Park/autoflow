...
- **[MUST]** A criterion that writing or running its test shows to be wrong, or that must promise a behavior it does not state, is raised in the RED report with the criterion, the proposed change and the fact that shows the need — never absorbed into the test — for the operator ([`autoflow-guide.md`](autoflow-guide.md) > RED step 2).
