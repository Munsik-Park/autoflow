...
- **[MUST]** Surface every issue found, including low-severity and uncertain ones — list them in `recommendations` (or `blocking_issues` when score-blocking). Severity and confidence are expressed through the `score` and `reason`, and on each recommendation (below), never by silently omitting a finding. The rubric score is the filter; the finding stage prioritizes coverage.
- **[MUST]** Each recommendation names its subject — a `path:line` at the evaluated commit, or a section of the cycle artifact it concerns (a design document, the DIAGNOSE analysis), which no commit carries — the item it was found under, and a severity on the reviewer's scale (`Critical` / `High` / `Medium` / `Low`), marked low-confidence when its evidence is weak. One at `Medium` or above also carries a `remedy_class` in the vocabulary of *Remedy class* below, answering the question the reviewer's findings are classified by: does clearing it discard or change a decision the deliberation settled (`design`), and if not, which kind of change clears it — `operator` when that cannot be said with confidence. The severity describes the finding and does not by itself lower the item. After a PASS the orchestrator handles the recommendations by the procedure a reviewer finding takes, so one without these fields cannot be routed: a report lacking them is rejected and the evaluator re-spawned, with the same cap (max 2) and escalation as an empty `fail_hypothesis` ([`autoflow-guide.md`](autoflow-guide.md) > GATE:QUALITY > *PASS recommendations*).
- **[MUST]** At a gate that receives recommendations an earlier gate raised and could not score — their fix lay ahead of it (GATE:PLAN for GATE:HYPOTHESIS's, GATE:QUALITY for GATE:PLAN's) — disposition every one `cleared` or `remains` with the ground re-derived from the work, in the `carried_recommendations` output field; a `remains` one is also listed at its severity in `recommendations` or `blocking_issues`. A report that leaves a carried recommendation unaccounted for is rejected and re-spawned, with the same cap (max 2) and escalation as an empty `fail_hypothesis`.
...
- **[MUST]** Score scope against those records, not against acceptance-criterion IDs alone. At GATE:PLAN, `Scope` reads the feature design's `## Scope` section. At GATE:QUALITY, `Minimal implementation` and `Impact scope` read the `## Scope` section, every `## Scope judgments` section in the cycle's `.autoflow/issue-{N}-*.md` reports, and the ledger's gate entries (Change Surface Rules > GATE:QUALITY linkage).
...
The FAIL tagging below binds the GATE:QUALITY form only; the same vocabulary classifies every
rubric-scored gate's `Medium`+ recommendations (*Finding coverage* above). The orchestrator routes a
FAIL's re-entry from this field ([`autoflow-guide.md`](autoflow-guide.md) > GATE:QUALITY > FAIL
routing), and a recommendation's fix the same way (same file > GATE:QUALITY > *PASS
recommendations*); the evaluator is the classifying authority and the implementing roles do not
re-classify.
...
**Automated (hermetic)** — `tests/test-issue-179-relay-state.sh` locks the relay's mechanical half over fixture transcripts: the transcript grammar, the Developer AI opening and strict alternation, the end-at-two-consecutive-`further: none` rule, a `### Brief` block re-opening the discussion, the report sections, fail-closed on a malformed transcript, and the metrics aggregation. `test/workflows/run.mjs` locks the two workflows' pure control-flow logic (ARCHITECT Record: the scribe call over the transcript file and the agreed-only ledger call; VERIFY next_action incl. `missing`; the spawn-policy fail-closed guards; the arg guards) by running each script against a mock runtime that injects only the real globals (`args`, `phase`, `parallel`, `agent`, `console`) — a stray undefined global throws, catching the ABI-regression class. Run `node test/workflows/run.mjs`; the GitHub Actions `workflow-regression` job (`.github/workflows/workflow-regression.yml`) runs it on every PR/push touching `.claude/workflows/**` or `test/workflows/**`, on `ubuntu-latest` where a node runtime is guaranteed — the Jenkins agent is provisioned for compose/shell validation only and carries no node, so the regression lives where the runtime exists (same rationale as `.github/workflows/image-scan.yml`). Enforcement is **advisory**: this repo's plan provides no GitHub branch-protection/required-status-checks (the API returns 403), so a red run does not auto-block merge — it surfaces a pass/fail signal that the external reviewer verifies green before merge (the "CI green" precondition in [`external-review-sequencing.md`](external-review-sequencing.md)), the same path as every other CI check.
