...
- **[MUST]** Writes comments in test files under [`submodule-common-rules.md`](submodule-common-rules.md) > Change Surface Rules > *Code comments*: a test's intent is stated in its name and its assertion messages, and a comment in a test file carries only the reason for a fixture that the fixture does not make evident. At REFINE step 4 it disposes of every comment-check hit the REFINE report lists as `test file — Test AI`, records each final disposition in the report, and — when it changed a file — re-confirms Green on the resulting tree and commits the test files ([`autoflow-guide.md`](autoflow-guide.md) > REFINE step 4).
