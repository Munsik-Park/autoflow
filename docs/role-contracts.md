...
- **[MUST]** A code comment carries only a sentence that stays true for as long as the code it sits
  ...
  judgment, and the fix is its direct commit ([`autoflow-guide.md`](autoflow-guide.md) > GATE:QUALITY > *Code comments in a target*).
...
- Spawn model: resolved from the spawn policy, never restated here — `bash scripts/spawn-policy/spawn-policy.sh model red` (and `refine-test-reconfirm` at REFINE). The row's tier moves only under the revert rule ([`CLAUDE.md`](../CLAUDE.md) > Spawn Model). Source: `.claude/autoflow/spawn-policy.json`.
...
- **ARCHITECT — Record, and VERIFY — the `Workflow` tool.** The `architect-deliberation` workflow is the Record phase: its scribe reads the transcript file in-script and writes the artifacts, its ledger call appends the agreed conclusions. The `verify-cause-branch` workflow runs both self-checks in-script.
...
The workflow's only output to the orchestrator is one structured result, **specific to the phase**:
...
- **Artifact existence is not the script's to check**: whether the scribe's files were written is checked by the orchestrator — see [`autoflow-guide.md`](autoflow-guide.md) > ARCHITECT.
...
After the result returns, the orchestrator **verifies** it before accepting:
...
| Plan evaluation | Feasibility, Scope, Security, Test plan (4) — affected files / side effects are derived at RED/GREEN, not scored here; Feasibility/Scope carry structural-fit & over-engineering across the plan and its verification design (not scored at DIAGNOSE; interpretation and embedded checks: [`autoflow-guide.md`](autoflow-guide.md) > GATE:PLAN); a re-entry re-scores the decision document's delta only | max 3× |
