...
- **[MUST] Resolve the anchor before executing.** Where the anchor being re-derived is a **suite
  ...
  `not-run` ([`autoflow-guide.md`](autoflow-guide.md) > GATE:QUALITY > *Test coverage* > *Execution
...
- Authors the verification design document: acceptance criteria → verification disposition (`automated` / `existing-coverage` / `delivery-check` / `manual` / `environment-dependent` / `none`) → method, with a one-line reason on every non-automated issue-AC row and a test kind (`driving` / `regression` / `characterization`) on every automated row — and its `## Tools` section: the tool each criterion needs, its availability and the ground for it, found before any criterion is settled as a `manual` row executed by a person, as `environment-dependent`, or on a mock. Rule body: [`autoflow-guide.md`](autoflow-guide.md) > ARCHITECT > Output artifacts > *Tools*; [`CLAUDE.md`](../CLAUDE.md) > Rule Scope > *The tools the work needs*.
- **[MUST]** Applies **Test necessity** to existence: a test exists only when it is needed — the burden of proof lies on the test, and the default under uncertainty is `none`. Retention is a separate question: `cycle` by default (uncommitted, run once from `.autoflow/issue-{N}-local/`), and a file added to the target's tree is the listed exception the reviewer judges — in the AutoFlow repository itself, a `standing` row under one of ADR-0024 D1's closed tokens (`automated / standing: <token>`). Rule body: [`autoflow-guide.md`](autoflow-guide.md) > ARCHITECT > Output artifacts > *Test necessity*.
...
- For untestable items: first finds the tool that verifies the item directly; only when none can be secured, states the reason and proposes alternatives (design change / a manual scenario executed by a person (except where the composition-oracle clause applies) / mock (same exception)).
- **[MUST]** At VERIFY step 1, performs each `manual` row whose executor is `AI: <tool>` with that tool and writes the row's observation record under `.autoflow/issue-{N}-local/` — what was looked at, how, the artifacts it left, the comparison against the referenced material, and the result line; the implementer does not certify its own result. A tool that neither this environment nor the target's procedures provide is reported to the orchestrator, never acquired. Rule body: [`autoflow-guide.md`](autoflow-guide.md) > VERIFY step 1.
...
- **[MUST]** Runs verification (jest/build) in the **foreground**; never uses `run_in_background` — a role spawn has no future turn to receive a background-completion notification. See [`role-common-rules.md`](role-common-rules.md) > Bash Execution Mode.
...
- Uses the tools its work needs — the ones the verification design's `## Tools` section records, and any the implementation itself needs — and, for a row verified with a tool, looks at its result with that tool while implementing; the row's evidence is the Test AI's observation record at VERIFY. A tool that neither this environment nor the target's procedures provide is reported to the orchestrator, never acquired ([`CLAUDE.md`](../CLAUDE.md) > Rule Scope > *The tools the work needs*; [`autoflow-guide.md`](autoflow-guide.md) > GREEN step 2).
...
- **[MUST]** Runs verification (jest/build) in the **foreground**; never uses `run_in_background` — a role spawn has no future turn to receive a background-completion notification. See [`docs/submodule-common-rules.md`](submodule-common-rules.md) > Testing Standards > Bash execution mode.
...
- If the prerequisites do not hold, the orchestrator escalates with the **specific** cause — version gap vs. local `/config`/`settings.json` disable vs. env var vs. managed-policy disable — rather than a generic "unavailable", and proposes the matching enable step. It does **not** fall back to running a deliberation in its own turn stream (the ARCHITECT relay is not that: the orchestrator receives one line per turn and no body).
- The orchestrator invokes `Workflow({ name: "architect-deliberation", args: { issue: "N" } })` at the end of the ARCHITECT relay (`relay-state.sh` says `next=record`) and `Workflow({ name: "verify-cause-branch", args: { issue: "N", failLog: "<path>" } })` at VERIFY — `failLog` is the failing run's log, or the observation record of a `manual` row verified with a tool ([`autoflow-guide.md`](autoflow-guide.md) > VERIFY step 1). Reference scripts: [`.claude/workflows/architect-deliberation.js`](../.claude/workflows/architect-deliberation.js), [`.claude/workflows/verify-cause-branch.js`](../.claude/workflows/verify-cause-branch.js).
- **ARCHITECT `args` contract** (issues #166, #179): `issue` is required and is the only argument. The orchestrator's `brief` — its preparation for a re-discussion, such as a narrowed topic, facts it verified in the meantime, or the prior report's path — is not a workflow argument: it is appended to the transcript as a `### Brief` block (`scripts/architect/relay-state.sh brief`), the participants answer it in the relay, and the scribe reads it where it sits.
- Skill-channel invocation (issue #14): each script's `meta.description` now states the required `args` contract (the only surface the harness exposes to a skill-channel caller — no `SKILL.md` exists for these workflows), and both scripts tolerate a prose-string `args` by salvaging `issue` inside the `JSON.parse` catch path via a three-tier rule (first hit wins): tier 1 a `#N` token, tier 2 an `issue N`-anchored number, tier 3 a bare digit run **only when exactly one is present** (two-or-more bare runs is ambiguous → fail loud rather than silently adopting the first). `issue` is the only key the salvage recovers. `failLog` deliberately has **no** prose fallback — a prose invocation of `verify-cause-branch` still fails loudly on the `failLog` guard. The object-form examples above remain the canonical orchestrator path and are unchanged.
- The workflows' internal sub-agents carry no model literal: each call site spreads its opts from the `workflow_sites` row the script loads at run time (`.claude/autoflow/spawn-policy.json`) — the ARCHITECT `scribe` / `ledger` sites and the VERIFY self-check sites — and the two relay participants resolve theirs from the `phases` rows through the readout, so every site is governed by the config alone. See [`CLAUDE.md`](../CLAUDE.md) > Spawn Model.

### Responsibilities

- **Keeps the deliberation out of the orchestrator's context**: at VERIFY the self-checks run inside the workflow and their exchange stays in script variables; at ARCHITECT the two persistent participants write every turn and report to the transcript file and return one line each, so the orchestrator holds the relay's decidable state and never a body.
- **[MUST]** The relay participants and the in-script sub-agents run all Bash **foreground**-only (`run_in_background` is orchestrator-only) — none of them has a future turn to receive a background-completion notification. See [`role-common-rules.md`](role-common-rules.md) > Bash Execution Mode.
- **Relays the discussion** under the Discussion Protocol ([`docs/role-common-rules.md`](role-common-rules.md) > Discussion Protocol) — UNDERSTAND → VERIFY → EVALUATE → RESPOND, devil's advocate on the first exchange, no groundless agreement. The participants take alternating single-participant turns on one topic; each reads the turns written since its last one and answers the other side's last turn, and **ADR conformance** is a first-exchange devil's-advocate axis: the exchange verifies the resolution conforms to any governing ADR. The VERIFY step's scope over the transcript (ADR-0023 D1): a fact cited with a `path:line` is verified for both participants; a participant reads a file to ground a claim of its own or to dispute a cited one. Non-scored — this is the ARCHITECT deliberation surface, not a gate.
- **Writes artifacts** to `.autoflow/issue-{N}-*.md` through the scribe, after the discussion: the feature design, the verification design and the deliberation report. It does not inline artifact bodies in its return. **[MUST]** The feature design carries the architecture decision layer only — decisions, constraints and rejected alternatives, citing the verification design's `Failure mode` column for each verification's failure mode rather than stating it; file rows, per-suite dispositions and oracle condition clauses are derived at RED/GREEN entry and are not written here ([`autoflow-guide.md`](autoflow-guide.md) > ARCHITECT > *Output artifacts*). **[MUST]** Only a cycle's **first** Record writes the documents whole; every Record after it reads the existing documents plus the turns appended since the previous Record and **appends a `## Delta — round <n>` section**, leaving unchanged text exactly as it stands (issue #192; the delta section is also GATE:PLAN's narrowed re-entry input).
- **Appends to the decision ledger** (`.autoflow/issue-{N}-ledger.md`): one entry per agreed conclusion, with grounds, under the authority `ARCHITECT agreed` (append-only).
...
```json
{
  "phase": "architect",
  "report": {
    "agreed": ["one line per design conclusion both participants accepted"],
    "unagreed": [{ "point": "", "developer_position": "", "test_position": "", "why_raised": "" }]
  },
  "artifacts": [".autoflow/issue-N-feature-design.md", ".autoflow/issue-N-verification-design.md", ".autoflow/issue-N-architect-report.md"],
  "transcript": ".autoflow/issue-N-architect-transcript.md",
...
- **Orchestrator routing** (procedure: [`autoflow-guide.md`](autoflow-guide.md) > ARCHITECT > *Report routing*): an empty `unagreed` goes to GATE:PLAN after the artifact-existence check; a non-empty `unagreed` is one orchestrator judgment — discuss further with a `brief` appended to the transcript, or stop and ask the user; an agreed conclusion that changes an acceptance criterion's content goes to the operator before GATE:PLAN. A non-null `stopped` is repaired and re-run.
- **Turn order (the relay, outside this workflow)**: the Developer AI opens on turn 1 and the Test AI answers on turn 2, alternating; each turn is one block `### Turn n — <side> [further: yes|none]` appended to the transcript file by its author, and the discussion ends when two consecutive turns both carry `further: none`. `scripts/architect/relay-state.sh state` prints `turns`, `next` and `ended` from the file — the turn count lives there, not in this return. Each participant's report is a `## Report — <side>` section of the same file; the scribe reads both.
- **`stopped` is infrastructure state, never a design outcome.** Its sentinels are `scribe missing` and the spawn-policy load sentinels `spawn policy load agent missing` / `spawn policy absent` / `spawn policy malformed` / `spawn policy row incomplete (<keys>)` / `spawn policy effort contract unusable (<field>)`. A run that stopped returns `report: null` and writes no artifact; a completed record returns `stopped: null` whatever its report says. The relay's own infrastructure states — a participant that appends no turn after one re-wake, a report section still missing after one re-wake — are handled by the orchestrator's procedure before this workflow is invoked ([`autoflow-guide.md`](autoflow-guide.md) > ARCHITECT > *Relay procedure*).
...
- **Artifact existence is not the script's to check**: the hosted Workflow runtime injects no filesystem access and rejects `import(` at parse time, so the script cannot observe whether the scribe's files were written. That check belongs to the orchestrator, which has a shell — see [`autoflow-guide.md`](autoflow-guide.md) > ARCHITECT.
...
- **Termination**: a single self-check round — each side answers once and `next_action` follows deterministically; there is no internal loop. Repeated VERIFY entries are bounded by the existing GREEN↔VERIFY round-trip cap (max 3) in [`CLAUDE.md`](../CLAUDE.md) > Flow Control.

**[MUST]** Neither return carries the turns, a duplicate dual report, or an artifact body — paths, the report or `next_action`, and one line only (mirrors [`docs/submodule-common-rules.md`](submodule-common-rules.md) > Reporting Format).
...
- It does **not** read the design docs cover-to-cover to *judge* them — that full read-and-score is GATE:PLAN's fresh Evaluation AI. The orchestrator instead **spot-checks targeted excerpts**: it pulls the specific `path:line` a returned decision rests on and re-derives the cited fact (`git show`, the summary line read in a cited log, `git show HEAD:<file>`). A wider read of a returned artifact is the orchestrator's judgment, recorded with its grounds ([`CLAUDE.md`](../CLAUDE.md#cost-control) > Orchestrator context discipline); what stays fixed is that such a read informs a spot-check, never the gate's verdict.

### Verification scenarios (manual)

**Automated** — none committed: this repository's CI keeps only deployment-level checks (ADR-0024 D1; issue #293). A change to the relay's mechanical half (`scripts/architect/relay-state.sh` — the transcript grammar, the Developer AI opening and strict alternation, the end-at-two-consecutive-`further: none` rule, a `### Brief` block re-opening the discussion, the report sections, fail-closed on a malformed transcript) or to either workflow's control flow (the Record scribe call over the transcript file and the agreed-only ledger call; VERIFY `next_action` incl. `missing`; the spawn-policy fail-closed guards; the arg guards) is verified by the one-shot run of the cycle that changes it, and that run's record goes into the PR body; both ship to targets, so their behavior in a target's sessions shows where they are stamped and a failure there comes back as an issue ([`CLAUDE.md`](../CLAUDE.md) > Rule Scope > *Local verification*).
...
- **Un-agreed report routes to the orchestrator's judgment, not to GATE:PLAN**: run a deliberation that ends with a point in `unagreed`; confirm the orchestrator decides between a re-discussion with a `brief` and a stop, and that GATE:PLAN is not spawned on that return.
- **Ledger carries `ARCHITECT agreed` entries only**: confirm one entry per agreed conclusion under that authority, and no entry for any un-agreed point.
...
| Plan evaluation | Feasibility, Scope, Security, Test plan (4) — affected files / side effects are derived at RED/GREEN, not scored here (issue #192); Feasibility/Scope carry structural-fit & over-engineering across the plan and its verification design (not scored at DIAGNOSE; interpretation and embedded checks: [`autoflow-guide.md`](autoflow-guide.md) > GATE:PLAN); a re-entry re-scores the decision document's delta only | max 3× |
