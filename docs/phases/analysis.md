...
**Review-response loop check** (`mode = review-response` only; runs at DIAGNOSE entry, ahead of the structure analysis below). An `autoflow-loopcheck` sub-agent (a shipped read-only definition; issue #180 replaced the harness Explore type so the row's effort is policy-governed), on the model the policy names for `diagnose-loopcheck` (clears the pre-GATE hook like Phase A/3), writes `.autoflow/issue-{N}-loopcheck.md` and returns a one-line summary. The contract has three separated steps:
