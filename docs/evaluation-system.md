...
with its `subject` — a `path:line` at the evaluated commit, or a section of the cycle artifact it
concerns (a design document, the DIAGNOSE analysis), which no commit carries — the
