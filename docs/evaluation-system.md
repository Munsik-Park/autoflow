...
triages the list by the reviewer-finding procedure — `Medium`+ routed through
`scripts/gate/remedy-route.sh` and re-scored by the same gate, `Low` judged, each attempt a
`[gate-autofix]` ledger entry ([`autoflow-guide.md`](autoflow-guide.md) > GATE:QUALITY >
