...
- Skill-channel invocation (issue #14): each script's `meta.description` now states the required `args` contract (the only surface the harness exposes to a skill-channel caller — no `SKILL.md` exists for these workflows), and both scripts tolerate a prose-string `args` by salvaging `issue` from the first `#N` token (falling back to the first bare digit run) inside the `JSON.parse` catch path. `failLog` deliberately has **no** prose fallback — a prose invocation of `verify-cause-branch` still fails loudly on the `failLog` guard. The object-form examples above remain the canonical orchestrator path and are unchanged.
