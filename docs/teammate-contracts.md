...
- **ARCHITECT `args` contract** (issue #166): `issue` is required; `brief` is an optional string — the orchestrator's preparation for a re-discussion, such as a narrowed topic, facts it verified in the meantime, or the prior report's path — carried verbatim into the topic on every turn. `brief` is accepted from a JSON / object argument only; a value that is neither a string nor absent throws at the boundary, at the same single throw site as a missing `issue`.
- Skill-channel invocation (issue #14): each script's `meta.description` now states the required `args` contract (the only surface the harness exposes to a skill-channel caller — no `SKILL.md` exists for these workflows), and both scripts tolerate a prose-string `args` by salvaging `issue` inside the `JSON.parse` catch path via a three-tier rule (first hit wins): tier 1 a `#N` token, tier 2 an `issue N`-anchored number, tier 3 a bare digit run **only when exactly one is present** (two-or-more bare runs is ambiguous → fail loud rather than silently adopting the first). `issue` is the only key the salvage recovers. `failLog` deliberately has **no** prose fallback — a prose invocation of `verify-cause-branch` still fails loudly on the `failLog` guard — and neither does ARCHITECT's `brief`, which is the orchestrator's own preparation and reaches the run as a JSON argument. The object-form examples above remain the canonical orchestrator path and are unchanged.
...
- **Relays the discussion** under the Discussion Protocol ([`docs/teammate-common-rules.md`](teammate-common-rules.md) > Discussion Protocol) — UNDERSTAND → VERIFY → EVALUATE → RESPOND, devil's advocate on the first exchange, no groundless agreement. The participants take alternating single-participant turns on one topic, each turn receiving the transcript so far, and **ADR conformance** is a first-exchange devil's-advocate axis: the exchange verifies the resolution conforms to any governing ADR. Non-scored — this is the ARCHITECT deliberation surface, not a gate.
...
- **Orchestrator routing** (procedure: [`autoflow-guide.md`](autoflow-guide.md) > ARCHITECT > *Report routing*): an empty `unagreed` goes to GATE:PLAN after the artifact-existence check; a non-empty `unagreed` is one orchestrator judgment — discuss further with a `brief`, or stop and ask the user; an agreed conclusion that changes an acceptance criterion's content goes to the operator before GATE:PLAN. A non-null `stopped` is repaired and re-run.
- **Turn order**: the Developer AI opens on turn 1 and the Test AI answers on turn 2, alternating. Each turn returns `{ message, done }` and receives one fixed prompt for its role, the topic, and the transcript of every prior turn; the transcript is the participants' memory and the run's only discussion record. The discussion ends when two consecutive turns both return `done: true`. `turns` counts the turns issued.
- **`stopped` is infrastructure state, never a design outcome.** Its sentinels are `participant missing for N consecutive turn(s)` (two consecutive null turns; a single null turn is recorded in the transcript as missing and the discussion continues), `report missing` (both participants' reports null — one null report is carried to the scribe as missing), `scribe missing`, and the spawn-policy load sentinels `spawn policy load agent missing` / `spawn policy absent` / `spawn policy malformed` / `spawn policy row incomplete (<keys>)` / `spawn policy effort contract unusable (<field>)`. A run that stopped returns `report: null` and writes no artifact; a completed deliberation returns `stopped: null` whatever its report says.
...
**Automated (mock-runtime regression)** — `test/workflows/run.mjs` locks the pure control-flow logic (ARCHITECT's relay order — the Developer AI opening and strict alternation — the end-at-two-consecutive-`done` rule, a single missing turn recorded in the transcript against two consecutive missing turns stopping the run, the report and scribe calls and the agreed-only ledger call, VERIFY next_action incl. `missing`, and the arg guards for `issue` and `brief`) by running each script against a mock runtime that injects only the real globals (`args`, `phase`, `parallel`, `agent`, `console`) — a stray undefined global throws, catching the ABI-regression class. Run `node test/workflows/run.mjs`; the GitHub Actions `workflow-regression` job (`.github/workflows/workflow-regression.yml`) runs it on every PR/push touching `.claude/workflows/**` or `test/workflows/**`, on `ubuntu-latest` where a node runtime is guaranteed — the Jenkins agent is provisioned for compose/shell validation only and carries no node, so the regression lives where the runtime exists (same rationale as `.github/workflows/image-scan.yml`). Enforcement is **advisory**: this repo's plan provides no GitHub branch-protection/required-status-checks (the API returns 403), so a red run does not auto-block merge — it surfaces a pass/fail signal that the external reviewer verifies green before merge (the "CI green" precondition in [`external-review-sequencing.md`](external-review-sequencing.md)), the same path as every other CI check.
...
- **Isolation**: run `architect-deliberation`; confirm the orchestrator transcript contains the single result object and **no** Developer/Test turns.
