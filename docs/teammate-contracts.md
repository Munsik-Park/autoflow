...
- A peer-teammate facilitator inside the orchestrator's own team is rejected on the ground recorded at [`docs/design-rationale.md`](design-rationale.md) > Decision 8, which cites the measurement in `tests/manual/issue-52-manual-scenarios.md` > M1. This bullet does not restate that ground. A facilitator **sub-agent** relaying persistent participants is not executable either: a woken participant's answer reaches only the session's main loop (ADR-0023, constraint 2).
- **ARCHITECT — the relay.** The orchestrator spawns the Developer AI and the Test AI once each as anonymous direct spawns (`subagent_type: autoflow-planner`, no `name`; models from the policy rows `architect-dev-participant` / `architect-test-participant`) and wakes them in alternation by agent ID with `SendMessage`, waiting for each answer by ending its turn. Each participant appends its turn to `.autoflow/issue-{N}-architect-transcript.md` and returns one line; `scripts/architect/relay-state.sh state` prints the next side and the end condition (two consecutive `further: none`), and the orchestrator obeys it. Each participant then appends its report to the same file. Isolation holds by the participants' prompt (`.claude/agents/autoflow-planner.md` > *ARCHITECT relay participant*: bodies to the file, one line back) and is checked by `scripts/architect/isolation-check.sh`. The procedure is [`autoflow-guide.md`](autoflow-guide.md) > ARCHITECT > *Relay procedure*.
...
- **ARCHITECT `args` contract** (issue #166, narrowed by issue #179): `issue` is required and is the only argument. The orchestrator's `brief` — its preparation for a re-discussion, such as a narrowed topic, facts it verified in the meantime, or the prior report's path — is no longer a workflow argument: it is appended to the transcript as a `### Brief` block (`scripts/architect/relay-state.sh brief`), the participants answer it in the relay, and the scribe reads it where it sits.
...
- **Isolation**: run one ARCHITECT relay to its Record; then `bash scripts/architect/isolation-check.sh .autoflow/issue-N-architect-transcript.md <session.jsonl>` must report every turn clean — the orchestrator's session log holds the one-line notifications, the `relay-state.sh` output and the single Record result, and **no** turn body (ADR-0023 D4).
...
| Plan evaluation | Feasibility, Scope, Security, Test plan (4) — `Dependencies` retired by issue #192 (affected files / side effects are derived at RED/GREEN, not scored here); Feasibility/Scope carry structural-fit & over-engineering across the plan and its verification design (not scored at DIAGNOSE); a re-entry re-scores the decision document's delta only | max 3× |
