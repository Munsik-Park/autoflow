...
- **Counter shape and the issue register**: a counter is a record `{ agenda, locator, argument, changes }` — the concern's name, where to look, the case itself, and (issue #166) what accepting it **changes**: an acceptance-criterion id, a verification layer, or a change-surface file, empty when none. The first three cross the turn boundary as an **issue register** entry `{ name, conclusion, evidence, status, raisedBy }`, held in a script variable for the whole run; `changes` is a raise-time classifier and is not carried — the entry's status records its outcome. An entry is keyed by its normalized short name and updated **in place** on a re-raise. An **open** entry is rendered into every later turn's prompt as exactly four labelled lines (`name` / `conclusion` / `evidence` / `status`) followed by a `---` terminator — a structural per-entry bound, with **no character cap and no truncation marker**; a **closed** entry (`agreed` / `rejected` / `observation`) is rendered by **name only**, in a per-status list under one declared heading, on cold and resume turns alike. Every entry carries every turn in one of those two forms, so a disposed issue stays visible and the no-re-litigation rule has something to point at. Because `conclusion` (the former `argument`) crosses the boundary, a Developer-AI counter's reasoning reaches the Test AI inside the record; the design document is not used as that durable channel.
- **Counter substance** (issue #166): from the third turn of a cold run on, and on every resume turn, a counter whose `changes` is empty is recorded under the status `observation` instead of `open` — kept by name, owed no disposition, never an escalation ground, and stated in the prompt as not a reason to withhold `accept`. A closed entry re-raised without a named change stays closed; an open entry re-raised without one stays open (it was substantiated when raised). The first exchange is exempt. The predecessor's counters are handed to the next turn partitioned the same way: the open ones as current counters to dispose of, the observations by name as owing nothing. Reconcile's minted acceptance-criterion entries name their change and therefore stay open, which the operator's re-entry depends on.
- **Disposition**: the script cannot judge agreement, so each side additionally returns `dispositions: [{ name, conclusion, evidence, status }]` with `status` in `{agreed, rejected}` — required, empty array permitted. A disposition acts only on an **existing, open** entry (an unresolvable name is ignored, never minted; a closed name is ignored, never re-applied — issue #166) and only with an in-enum status; names resolve through the same normalization as register keys. The turn schema offers `name` as the enum of the register's open names whenever any entry is open, so the channel is told the same set the runtime guard admits. **Only the entry's raiser may close it** — a disposition from the other side overwrites `conclusion`/`evidence` as a proposed resolution but leaves `status: open`.
...
- **Record discipline**: the design documents carry only the current design and its conclusions — no round history, no copy of the register, no measurement logs, command output or code text transcribed into them; evidence is one line of what was checked and how, re-verified next round with tooling. Issues are named with short readable names, never serial numbers, and no totals or counts are written into the documents. The check is mechanical (issue #166): `scripts/architect/record-discipline.sh check` reports the residue classes by grammar; a Draft or Converge sub-agent that wrote or edited a document runs it before returning and clears its own hits, and the orchestrator runs it as an advisory after a `CONVERGED` return ([`autoflow-guide.md`](autoflow-guide.md) > ARCHITECT > *Record rules*).
- **Read by diff** (issue #166): each side's turn snapshots both documents at its start (`.autoflow/issue-{N}-architect-snapshot-{side}-*.md`, a command the sub-agent runs — the script has no filesystem); a side's first cold turn reads in full, every later turn and every resume turn diffs against its side's snapshot first and reads the changed hunks, falling back to a full read when the snapshot is absent.
