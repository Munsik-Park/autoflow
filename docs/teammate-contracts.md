...
- **Counter shape and carry payload**: a counter is a record `{ agenda, locator, argument }` — the concern's name, where to look, and the case itself. Only `agenda` + `locator` cross a round boundary (capped by the script at 80 / 120 code points with a truncation marker); `argument` is consumed in the round that wrote it. A Developer-AI counter's reasoning therefore reaches the Test AI through a named open-concern entry in the feature design document, not through the carry channel. Terminal ledger grounds keep the full records.
