...
- Default spawn model: `opus` at GREEN and VERIFY (implementation surface, self-check sycophancy risk). REFINE uses `sonnet` (mechanical `/simplify` application) — spawned fresh at REFINE entry, since each phase's spawn selects its own model.
...
- **Resume guards and the resume round bound** (issue #127): a resume run loads the register before Converge, and each failure resolves to its own bare `escalation` sentinel — `resume register load agent missing`, `resume register absent`, `resume design artifact missing`, `resume register has no open entry`, `resume register already converged`, `resume register still open at convergence`. The last is not a load guard but the terminal outcome of the resume-only convergence condition above — it is ordered after the infrastructure sentinels (a missing closing agent still outranks it) and before the generic round-exhaustion text, so the operator reads which register entry blocked the run instead of a round count; the `ESCALATE` ledger grounds render those entries. Artifact existence **is** checked on this path, because resume skips Draft and the load sub-agent is already reading the filesystem the script cannot reach. Round numbering continues from the register's `lastRound`, never from an argument, and the run admits exactly one further round — which may still converge through the closing half-round when the Developer AI's response is a grounded ACCEPT **and** that closing turn leaves no register entry open; a converged resume returns `CONVERGED` only when the Reconcile check that follows finds no unauthorized acceptance-criterion change, and `AC_CHANGE` when it does. A both-null round on that bound is a MISSING judgment immediately (`sub-agent missing for 1 consecutive round(s)`) rather than falling through to the generic round-exhaustion text: with only one round admitted, no further round can retry it. The resume round's agenda is the **open** entries; `agreed` and `rejected` entries are carried under a declared settled-block heading as a record, not as work — after an `ESCALATE` the ledger holds no record of what the prior run rejected, so deleting them would reopen re-litigation through the back door.
...
- **ARCHITECT**: a *round* = one Developer-AI ↔ Test-AI exchange cycle. Max **6 rounds** (**2** on a scope-bounded review-response run, `args.bounded` — issue #135; the closing half-round and the resume extension apply to that ceiling unchanged). Because a round runs Test-AI-then-Developer-AI, round 6 would otherwise end on a Developer-AI revision no Test-AI turn has evaluated: when that revision is a grounded ACCEPT, a **closing half-round** runs — one Test-AI-only evaluation of it. It is the second half of the sixth exchange, not a seventh round, so it adds no Developer-AI turn and does not increment the returned `rounds`; the round definition above is unchanged. Its verdict decides the run: a grounded ACCEPT converges it — returning `verdict: "CONVERGED"` when the Reconcile check that follows finds no unauthorized acceptance-criterion change and `verdict: "AC_CHANGE"` when it does — a COUNTER/PARTIAL returns `verdict: "ESCALATE"` with the round-exhaustion reason, and a null closing return is a MISSING judgment carrying its own distinct `escalation` reason. On no mutual ACCEPT at round 6, the workflow returns `verdict: "ESCALATE"`. (Encoded as `MAX_ROUNDS = 6` in the script.)
