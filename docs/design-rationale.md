...
**Termination of the ARCHITECT deliberation — turn model** (issue #152)

The ARCHITECT convergence loop is turn-based: the two participants alternate single-participant turns, each reporting only `modified` (did this turn edit a design document) and `accept` (does this participant accept the current design), and the loop terminates in exactly one state — two consecutive turns both reporting `modified: false, accept: true`. This replaced a verdict-history protocol (`ACCEPT/COUNTER/PARTIAL` responses, grounded-ACCEPT structural requirements, a cap-round closing half-round, and a resume-only open-entry convergence guard) whose machinery existed to verify after the fact that both sides had accepted the *same* proposal. The turn model makes that verification unnecessary by construction: a modification keeps the conversation open — the modified design naturally returns to the other participant on the next turn — so no snapshot hash, revision pin, acceptance-invalidation rule, or closing evaluation is needed. The accepted trade-off is at most one extra turn in cases where agreement is already obvious.

The bound this decision requires is unchanged in kind: the turn ceilings are config values (`.claude/autoflow/spawn-policy.json` > `deliberation_caps` — cold and scope-bounded), the loop exits on its own bound every invocation, and reaching the ceiling without convergence escalates to the operator.

**Termination under an ARCHITECT resume** (issue #127; turn model per issue #152)

The ARCHITECT facilitation workflow accepts a `resume` argument that re-enters an `ESCALATE`d deliberation from its persisted register instead of cold-restarting it. Each resume admits exactly one further exchange and is unbounded in how many times an operator may take it, which reads at first like the loop-without-a-termination-condition this decision forbids. It is not, and the distinction is the one this decision actually turns on: **the loop's bound is in code, and every invocation carries one.** A resume run computes its ceiling as the persisted `lastTurn + 2` and admits exactly one exchange (two turns); the loop then exits on its own bound. There is no path on which a single invocation runs unbounded, and no counter is carried in the register that a later run could inflate. The resumed run's first turn has no predecessor to pair with, so a resume converges only on its own two turns — the escalated run's final turn state is deliberately not restored, which costs at most the same one extra turn the turn model already accepts.

The resume admission reads the register's `lastResponses` for exactly one shape (issue #159): both sides accepting with no entry open — the state a deliberation reaches when its participants agree but each accepting turn still edits a document, so under the turn model no unmodified-accept pair ever forms. Refusing that register (`no open entry`) left only a cold restart, which re-authors both designs to confirm a design nobody disputes; #157 cycle 1 measured that cost at roughly 1.9M tokens and 76 minutes and was worked around by hand-editing the facilitator's register. The admission is the workflow's, not the operator's: the exchange it admits is still bounded at `lastTurn + 2`, its two turns decide by the unchanged pair condition, and the register stays facilitator-owned. The complementary rule is stated to the participant on every turn — accepting means not editing — so the state is reached less often in the first place.

What is unbounded is the number of *operator decisions* to spend one more exchange — and an operator decision is precisely the human escalation point this decision requires a loop to terminate at. The termination condition is satisfied by the bound, not by a count of how many times a human chose to continue past it; requiring the latter would mean a human who has read the register and judged one further exchange worthwhile is refused by a machine cap. The pathological case this decision guards against — a loop that never hands control to a human — is the opposite shape: here control returns to the human on **every** extension.
...
The realization matters because the obvious alternative does not exist: in Claude Code Agent Teams a spawned teammate cannot create its own team and the lead is fixed for the team's lifetime, so "a facilitator that leads a nested team" is not executable; and a peer-teammate facilitator was rejected on a ground this repository has since withdrawn: peer-to-peer content was measured *not* to enter the lead's stream. The rejection stands on the ground that survives — the `Workflow` mechanism enforces the turn ceiling and the convergence condition in code, whereas a peer facilitator would carry them only in prose. The peer-facilitator alternative is recorded as reopened, not adopted. Observed, not guaranteed — reproduce via `tests/manual/issue-52-manual-scenarios.md` > M1. The `Workflow` runtime is the mechanism whose isolation is actually documented, so the contract binds to it rather than to an abstract "sub-context".
