...
| the ten partially-touched suites' doc-STATE assertions (`846`, `847`, `848`, `955`, `798`, `799`, `985`, `adr-0016`, `16`, `795`) | **migrated → registry entries**, the assertion removed from its suite; the suite itself retained | each moved occurrence is recorded in `tests/fixtures/issue-76-migration-map.md` against the registry `id` that now carries it, and every occurrence that did **not** move is recorded there too, against a §6.1 disposition naming why it is not registry-expressible. The split is not by suite but by shape: a single-file, case-sensitive, single-conjunct predicate over a whole file or a resolvable anchor moves; a diff, an exit status, a count, a JSON query, a computed region, a case-insensitive match, an `absent` ERE, or a predicate spanning two files stays where it is. Migrating an inexpressible region under whole-file scope instead would discard the scoping that IS the assertion — the ground on which `844` was held unmigratable until `section_kind` landed |

### 6.1 Disposition labels for the issue #76 migration map

`tests/fixtures/issue-76-migration-map.md` accounts for **every** assertion
occurrence of every touched suite exactly once, migrated or not. An occurrence
that moves names its carriers; an occurrence that **stays where it is** names a
disposition from the table below, with the reason it is not registry-expressible.
The labels are the greppable left-hand side those rows cite.
