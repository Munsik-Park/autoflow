...
| `tests/test-issue-85-disposition-record.sh` | **cycle-scoped** | it asserts this cycle's own landed state — the agreement between the rows above and the tree — which is §2's second criterion. Issue-numbered by the naming rule, so its lane is legible from the file name. Its file, its `run:` step, its `paths:` entries and this row are removed together in the cycle's final commit before the DELIVER push, per §2's execution point. Its Green evidence is the VALIDATE-time local run recorded before that commit: no hosted run ever executes it, because the PR head contains neither the file nor its registration |
