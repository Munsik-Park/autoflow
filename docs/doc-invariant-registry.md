...
| test-issue-51-teammate-removal-verdict.sh `AC1(a)` | **dropped — count fence** | seven `count_heading … -eq 1` checks. A count-shaped predicate is a cycle-scoped guard by §1-2 and can never be a permanent entry. **What is lost**: a duplicated `##` heading in the ADR is no longer caught. **What still holds**: the six `Decision`-anchored entries and the four `Q3`-`Q6` entries all resolve through the runner's own load-time gate, which BLOCKs the whole run on an ambiguous anchor — so a duplicated *anchored* heading still reds, and only an unanchored duplicate goes unguarded |
| test-issue-51-teammate-removal-verdict.sh `AC2` | **dropped — count fence** | two legs, both dropped. The heading-count leg is a count fence (as `AC1(a)`); the grounds-anchor leg asserted a per-section property (*each answer carries at least one path / issue / SHA anchor*) that no single entry states — an entry pins a literal, not a schema over six sections. **What is lost**: an ungrounded answer added to any of Q1-Q6 no longer reds. **What still holds**: the four promoted `Q3`-`Q6` content entries |
...
| test-issue-51-teammate-removal-verdict.sh `AC14` | **dropped — count fence** | seventeen `count_heading … -eq 1` checks over the full skeleton. Count fence, as `AC1(a)`. **What is lost / still holds**: as `AC1(a)`, over the wider heading set |
...
their rows above state each loss individually. That is a larger recorded loss than the
