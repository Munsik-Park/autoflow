...
// procedure: docs/phases/architect.md). This workflow reads that file through its
...
if (!stopped) {
  ...
  const scribeReport = await Promise.resolve()
    .then(() => agent(
      `You are the scribe of an AutoFlow ARCHITECT deliberation. The discussion has ended; its whole record is the relay transcript file ${transcript}: the "## Topic" section (the question and any orchestrator brief), every "### Turn n — <side>" block in order (any "### Brief" block re-opened the discussion with the orchestrator's preparation), and the two participants' reports under "## Report — Developer AI" and "## Report — Test AI" (each lists the conclusions that participant accepted and the points it raises as un-agreed). A "### Brief" block after a pair of report sections opens a new round — a re-discussion after GATE:PLAN or after an un-agreed point — and the round's own report sections follow its turns: the participants' conclusions are the last round's two reports; earlier rounds' reports are the record of what was re-opened. Before reading, check whether ${feature} already exists, because the answer decides which part of the transcript you read and whether you write the documents whole or append a delta to them (docs/phases/architect.md > Record). If it does not, this is the cycle's first Record: read the whole transcript and write the documents whole from it. If it does, a previous Record already wrote them: read the existing design documents and, of the transcript, only what was appended after the previous round's report sections — this round's "### Brief", its turns and its two report sections — locating those report sections from the transcript's heading lines; then append to ${feature} and to ${verif} a section headed "## Delta — round <n> (<why this round opened: GATE:PLAN FAIL | un-agreed re-discussion | VERIFY design contradiction | design re-entry | acceptance-criterion decision | gate recommendation>)" with one bullet per change — what changed, the decision as it now stands, and which section or decision it supersedes — and write the report's "## Agreed" and "## Unagreed" for this round. Leave every sentence the round did not change exactly as it stands — do not re-author, re-word or re-order settled text — because the delta section is what GATE:PLAN re-scores on re-entry. In either branch, write three files in the form docs/phases/architect.md > Output artifacts defines for each. (1) The Feature Design Document to ${feature} — the architecture decision layer only (docs/phases/architect.md > Output artifacts, item 1): the decisions the participants reached, the constraints each holds under, and the alternatives they considered and rejected with the ground for each rejection. Do not state here the failure mode a verification catches: that is the verification design's Failure mode column, which this document cites rather than restates. Do not write a change table of files, a per-suite disposition, or an oracle's condition clause — those are derived at RED/GREEN entry by the execution roles, and writing them here is a defect (docs/phases/architect.md > Output artifacts). The test for every sentence: if it were wrong, would the design have to be revisited, or would it just be fixed where it is found? Write only the first kind, which is what belongs to the deliberation (docs/phases/architect.md > Output artifacts, item 1). The document carries a "## Scope" section: each problem beyond the acceptance criteria that the participants judged under docs/submodule-common-rules.md > Change Surface Rules > Scope judgment, as included or separated, with the condition it meets and, for a directly related problem left out, its separation reason — "none" when nothing lies beyond the criteria. (2) The Verification Design Document to ${verif} in the format at docs/phases/architect.md > Output artifacts: one row per verification of each acceptance criterion of .autoflow/issue-${issue}-phase-b.md — a criterion verified more than one way gets one row per verification, every other criterion keeps its one row — plus a row whose Issue AC is — for each problem the Scope section includes, with its Issue AC id, verification disposition, kind, method, failure mode (the Failure mode column that section defines, the only place in either document a verification's failure mode is stated) and a one-line reason on every non-automated row, plus the Test necessity, Verification depth and Composition oracle determinations that section defines, and the "## Tools" section docs/phases/architect.md > Output artifacts > Tools defines — one line per tool the participants found: the tool, the rows it verifies, its availability (available | target procedure: <document section or script> | operator: <what is needed>) and the ground for it, or "none" with its ground in one line. Record the Composition oracle determination as the one composition-oracle block whose grammar docs/phases/architect.md > Output artifacts > Composition oracle defines — T and S as the Test AI's report identifies them, an empty list carrying its none declaration — and once ${verif} is written, run \`bash scripts/architect/composition-oracle.sh ${verif}; echo "exit status: $?"\` from the repository root and attach its stdout and exit status below the block verbatim, exactly as the shell produced them: never re-typed, summarized or re-derived, and never inside a composition-oracle fence. On a delta round that changes T or S, restate the whole block as the payload of one supersedes bullet, run the script again and attach that output in the delta section, because the script evaluates only the latest block (docs/phases/architect.md > Output artifacts > Composition oracle). (3) The report to ${reportPath}, the artifact the orchestrator routes (docs/phases/architect.md > Report routing), under the headings "## Agreed" (one line per agreed conclusion) and "## Unagreed" (per point: the point, the Developer AI's position, the Test AI's position, why it was raised). Record what the participants concluded and add no design of your own, since the design documents and the report state what the participants reached and what they did not (docs/phases/architect.md > Record); where the two reports differ on a point, carry it as un-agreed; if a participant's report section is missing, record that participant's positions from its turns within the part of the transcript your branch reads. Do not edit ${transcript}: the file is append-only once the relay creates it (docs/phases/architect.md > Relay procedure, step 1). Then return the consolidated report, the part of this run that reaches the orchestrator (docs/phases/architect.md > Record): "agreed" — the conclusions both reports list; "unagreed" — every point either report raises.${FOREGROUND_RULE}`,
