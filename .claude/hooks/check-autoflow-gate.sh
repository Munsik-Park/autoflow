...
# MALFORMED state, fail-closed before any gate), and the reader still fails
# closed on its own if it meets a non-string or empty value.
block_if_open_reentry() {
  local action=$1 phase_key=$2 _open
  if ! _open=$(printf '%s' "$STATE_JSON" | jq -r --arg phase "$phase_key" '[.. | objects | select(has("phases")) | select(.phases | has($phase)) | .phases[$phase].remedy_class] | last | if . == null then "" elif (type == "string" and length > 0) then . else error("malformed remedy_class") end' 2>/dev/null); then
...
if [ "$TOOL_NAME" = "Bash" ] && printf '%s' "$SCAN" | grep -qE "${CMD_BOUNDARY}${GIT_COMMIT}"; then
  if ! _remedy=$(printf '%s' "$STATE_JSON" | jq -r '[.. | objects | select(has("phases")) | select(.phases | has("gate_quality")) | .phases.gate_quality.remedy_class] | last | if . == null then "" elif (type == "string" and length > 0) then . else error("malformed remedy_class") end' 2>/dev/null); then
