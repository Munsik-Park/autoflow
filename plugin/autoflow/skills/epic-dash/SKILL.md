...
수집 결과는 gh 경로와 **동일한 JSON 스키마**로 `.autoflow/issue-analysis/` 에 저장한다.
`meta.json` 의 `source` 에 `mcp` 를 기록한다.
...
> **결정적 파이프라인 (Step 2~6).** Step 2~6은 체크인된 결정적 스크립트로 구현되어 있다. fetch 후:
> ```bash
> # 스크립트 위치를 설치 채널에 무관하게 해석한다.
> for S in \
>   "${CLAUDE_PLUGIN_ROOT}/skills/epic-dash/scripts" \
>   "$PWD/.claude/skills/epic-dash/scripts"; do
>   [ -d "$S" ] && break
> done
...
> 사양이며, **`선행 의존(deps)` 관례가 없는 레포에서는 Step 3의 LLM 추론으로 대체**한다.
...
- 라벨 epic과 충돌하면 서브이슈의 `{parent, children}` 를 우선한다.
...
라벨·태스크리스트 같은 명시적 신호는 **출발점일 뿐**, 그것만으로 끝내면 안 된다. 반드시 각 이슈의 `body` 와 `comments` 를 읽고
**내용 기반으로 선후관계를 추론**한다.
...
아래 A(명시적)·B(맥락 추론) 신호를 **합쳐** 엣지 집합을 만든다.
...
- 이슈 본문에 `- **선행 의존(deps)**: \`#67-S2b\`·hard, \`#64\`·mockable` 형태로 **명시 선언**된 의존은
  deterministic 추출한다(추론으로 덮어쓰지 않는다).
...
- **[MUST] 에픽 트래커를 향한 `hard` 선행은 그 에픽의 `S0` 셸 슬라이스로 해석한다.** 예: `#62·hard` → `#62-S0`=#281.
  `mockable` 의 트래커 참조(#64·#71 등 인터페이스)는 그대로 둔다.
- 구현: `scripts/extract_deps.py`. (B-1 의 `선행 의존: #N` 비구조 패턴보다 우선.)
...
- 같은 트랙에서 `due_on` 이 빠른 마일스톤의 이슈가 선행. 위상 정렬에서 하드 엣지로 쓰지 않고
  동순위(tie) 정렬 힌트로만 사용한다.
...
```json
{
  ...
  "edges": [
    ...
    { "from": 89, "to": "example-org/infra#12", "confidence": 1.0,
...
**독립적·병렬로 실행 가능**하고, 다음 Wave는 이전 Wave가 끝나야 시작된다.
...
1. 각 오픈 이슈의 in-degree 계산. 단:
   ...
   - `external:true` 인 `to`(서브레포)는 **미충족(차단)** 으로 카운트한다.
   ...
   - **`hard` 엣지는 같은 epic이든 다른 epic이든 모두 in-degree에 포함한다**(cross-epic 차단 포함). 단
     트래커 대상 `hard` 는 Step 3 A-5에 따라 `S0` 슬라이스로 치환된 뒤라야 카운트된다. 구현: `scripts/build_pipeline.py`.
...
- **[중요] 동결(`priority:low`) 슬라이스는 `work_nodes`에서 제외**되어 Wave 위상정렬과
  착수 추천에 들어가지 않는다(백로그로 보존, 별도 ❄️ Deferred 밴드로 렌더).
...
이 화면은 **"다음 작업 추천 시스템"이 아니라 프로젝트 현황 대시보드**다. 개별 추천보다
**전체 맥락을 우선** 제공하고, 사용자가 현재 상태를 이해한 뒤 스스로 무엇을 할지 판단하게 한다.
...
5. **의존성 그래프는 단순하게** — 복잡한 DAG 자체를 과시하지 않는다. **진행 중 Epic 중심**으로 단순화한다.
...
별도 섹션으로 빼내지 말고, 같은 카드 안에서 **색만 다르게(`done` 초록)** 표시한다. 권장 그룹 순서:
...
아래 레이아웃을 그대로 따른다(좁은 노드-링크형으로 그리지 말 것).

**[MUST] SVG 앞에 HTML 미니 범례 스트립을 반드시 배치한다**

그래프 `<div class="graph-wrap">` 안, SVG 바로 위에 다음 범례 HTML을 삽입한다.
범례는 **스크롤 없이 그래프 옆에서 읽을 수 있는 위치**에 둔다.
...
- **[MUST] 밴드 라벨은 위치가 아니라 그 밴드 노드들의 실제 상태 기준으로 정한다.** "첫 오픈 밴드 = 즉시 착수"로
  단정하지 않는다. 밴드에 `ready` 노드가 하나라도 있으면 "즉시 착수 가능 (Ready)",
  아니면 "선행 완료 후 (Blocked)"로 적는다.

**노드 박스 — 상태 배지 텍스트 필수**

박스 높이 **46px**, 텍스트 3줄 + 하단 상태 배지로 구성한다. 박스 안에 반드시 상태 텍스트를 표시한다.
...
- **cross-epic 엣지(다른 에픽 선행)** 는 화살표로 그리지 않고 그래프 상단에 노트로 표시한다:
...
⑤ **cross-epic `hard` 의존이 있으면 `⛔ cross-epic 선행(차단)` 노트가 떠 있고, 후행 노드가 대기(회색)인가** — 트래커 대상 의존이
   조용히 사라져 후행이 "즉시 착수"로 오표시되지 않는지 확인.
...
**완료된 Epic은 절대 대시보드에서 제거하지 않는다.** "✅ 완료된 Epic" 섹션에 **항상 표시**한다. 진행 중 Epic과 시각적으로만 구분(완료 색/체크)하고
숨기거나 누락시키지 않는다. 다음을 반드시 포함한다:

- Epic 제목 + ✅ 표시, 완료 색상(`stroke #238636, fill #1a4429`)
- **진행률 바 100%** 와 `{완료수}/{전체} 완료` 카운트
- 서브이슈 번호 chips (closed 표시)
...
**완료 Epic을 빼고 그리면 안 된다.**

---

## Step 6: 착수 순서 추천 + 근거 (LLM 핵심 산출물)

의존성 분석을 했다면 "이 순서로 하면 좋다"는 **추천과 그 근거**가 반드시 나와야 한다.
...
- `deps.json` 의 `analyzed_at` 을 헤더에 표시한다
- confidence·`type`·reason 을 반드시 HTML 툴팁/주석에 포함한다
...
- 서브이슈/마일스톤 엔드포인트는 레포·플랜에 따라 없을 수 있다 → best-effort, 없으면 태스크리스트
  파싱으로 대체하고 분석을 계속한다
- Wave 1 추천은 제안일 뿐 자동 착수가 아님을 명시한다
