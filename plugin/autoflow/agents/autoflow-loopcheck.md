...
description: AutoFlow DIAGNOSE review-response loop-check spawn — compares the trigger's complaint class — a review comment, or a gate's PASS recommendation — against the immediately-prior observation and returns match / no-match. The subagent_type IS the role declaration the gate hook reads (analysis role — never score-gated). Read-only tool set; replaces the harness Explore type for this row so its effort is policy-governed (issue #180).
...
- Read only the documents your prompt hands you (DIAGNOSE context separation);
  the immediately-prior ledger observation and the trigger — a review comment,
  or a gate's PASS recommendation — are the inputs.
