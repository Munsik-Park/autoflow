...
| GATE:PLAN / AUDIT / GATE:QUALITY (PASS, a recommendation disposed `fix`) → owning phase → that gate's re-score | the orchestrator disposes of each recommendation within the cycle's scope as `fix` / `reject` + reason / `operator` on the two scope-judgment questions, in one ledger entry. A `fix` travels the route its class names (`scripts/gate/remedy-route.sh route`; at GATE:PLAN, the DISPATCH spawn prompt for an item below the decision layer, an ARCHITECT re-discussion for a decision) and ends in that gate's re-entry re-score over the items the fix touched. One disposition pass per gate per cycle; it is not a FAIL and consumes no FAIL cap — a failing re-score is an ordinary FAIL, and a route through ARCHITECT consumes the ARCHITECT re-entry counter (`docs/autoflow-guide.md` > GATE:QUALITY > *Recommendation disposition*) |
