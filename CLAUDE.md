...
**Local verification** (the instance of principle 2 the phases cite). A change runs, locally and once, the tests the working AI judges it requires — at minimum every `automated` row this cycle authored or changed and every `delivery-check` row (ADR-0024 M) — through the target's declared test command, and records the command and its summary line (Reporting Format item 5). Regression verification is HANDOFF's CI (step 5). There is no local whole-tree run: none scheduled, none held in reserve. The declared test command is discovered in this order, first hit wins: `.claude/autoflow.local.json` > `tests.command`, then the target's `CLAUDE.md` > Development Commands `Test` entry (ADR-0024 D3); AutoFlow invokes it as declared and adjudicates nothing beyond its exit status. AutoFlow's own suite plane — the header contract, the selector and the runner — applies only where the target opted in (`.claude/autoflow.local.json` > `tests.suite_plane: true`) and in this repository, whose standing layer depends on it (ADR-0024 D5). A cycle-layer asset — a default `automated` row's test, a `delivery-check`, a `manual` checklist — lives under `.autoflow/issue-{N}-local/`, is never committed, and is archived with the issue's other artifacts (ADR-0024 D2).
