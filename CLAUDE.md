...
- **Phase-boundary respawn**: ARCHITECT's two relay participants live for one discussion and are not woken again once the Record `Workflow` has returned (ADR-0023 D2; there is no facilitator or role spawn to shut down). At DISPATCH entry the orchestrator spawns fresh agents for RED/GREEN, passing `.autoflow/issue-{N}-*.md` paths only — discussion history is not carried into implementation phases.
