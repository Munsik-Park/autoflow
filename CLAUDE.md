...
| DIAGNOSE (structure eval) → DIAGNOSE (cause) | GATE:HYPOTHESIS structure PASS (code change required) + its recommendations triaged (no `Medium`+ open) |
...
| GATE:HYPOTHESIS → ARCHITECT | cause analysis PASS + code change required + its recommendations triaged (no `Medium`+ open — a `Medium`+ one is amended in DIAGNOSE and re-scored by the same form) |
...
| GATE:PLAN → DISPATCH | plan evaluation PASS + its recommendations triaged (no `Medium`+ open; a `doc` / `test` / `impl` one is carried into the DISPATCH spawn prompt) |
...
| AUDIT → GATE:QUALITY | security audit PASS + its recommendations triaged (no `Medium`+ open) |
| GATE:QUALITY → DELIVER | completion evaluation PASS + its recommendations triaged (no `Medium`+ open) |
| GATE:HYPOTHESIS / GATE:PLAN / AUDIT / GATE:QUALITY (PASS, a `Medium`+ recommendation) → doc commit / RED / GREEN / ARCHITECT / DISPATCH carry / DIAGNOSE → that gate's re-score | a PASS report's `recommendations` are triaged by the reviewer-finding procedure of HANDOFF step 6.5, not by a procedure of their own (`docs/autoflow-guide.md` > GATE:QUALITY > *Recommendation triage*): the evaluator puts on every item a subject and a severity and on every `Medium`+ item a `remedy_class`; `scripts/gate/remedy-route.sh route` picks the phase — one behind the gate is re-entered and the recommending gate re-scores on its narrowed input (every route at AUDIT / GATE:QUALITY; `design` at GATE:PLAN → ARCHITECT re-discussion); one ahead of the gate receives the recommendation as input (a `doc` / `test` / `impl` at GATE:PLAN → the DISPATCH spawn prompt, scored by GATE:QUALITY); at GATE:HYPOTHESIS every class → DIAGNOSE amends the analysis → same-form re-score. A recommendation none of the three direct-relation conditions covers is separated with its ground (a separate issue the follow-up); the same four pause criteria as step 6.5 apply, (a) recording the operator's answer as an `[ac-decision]` entry. `Low` → orchestrator judgment (fix now, or defer with a known-gaps line in the PR body). Each `Medium`+ attempt is a `[gate-autofix]` ledger entry and the routed class is recorded as `phases.<gate>.remedy_class` while the attempt is open — the hook denies `git push` / `gh pr create` on its presence at `audit` / `gate_quality`, and the Resume procedure resumes on it. Cap 7 consecutive attempts since the last user decision → pause; not a FAIL, no FAIL cap consumed; a failing re-score is an ordinary FAIL; a route through ARCHITECT consumes the ARCHITECT re-entry counter |
...
**Remedy class recording** (a GATE:QUALITY FAIL, and an open `Medium`+ recommendation attempt at any gate): the orchestrator writes the routed class — the farthest of the evaluator's per-item classes, or `operator` — as `phases.<gate>.remedy_class` (a sibling of `evaluator` and `scores` inside the phase object, the one additive placement the validator admits; never top-level, never inside `scores`). The value is **removed** once the re-score PASSes with nothing `Medium`+ left open, or is left behind by a later cycle's record of that gate. The hook reads `gate_quality`'s value for the `git commit` gate below, and the presence of a value at `audit` or `gate_quality` for the `git push` / `gh pr create` gates — an open re-entry is not pushed past (`docs/autoflow-guide.md` > GATE:QUALITY > *Recommendation triage*).
