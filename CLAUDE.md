...
REFINE and raises in its report; or a gate recommendation the orchestrator disposes of as `operator`
([`docs/autoflow-guide.md`](docs/autoflow-guide.md) > ARCHITECT > *Report routing*, > GATE:QUALITY >
*Recommendation disposition*). The change excludes, revises or splits a criterion, or adds one. The
...
| DIAGNOSE (structure eval) → DIAGNOSE (cause) | GATE:HYPOTHESIS structure PASS (code change required) + its recommendations disposed |
...
| GATE:HYPOTHESIS → ARCHITECT | cause analysis PASS + code change required + its recommendations disposed (a `fix` item is amended in DIAGNOSE and re-scored by the same form) |
...
| AUDIT → GATE:QUALITY | security audit PASS + its recommendations disposed |
| GATE:QUALITY → DELIVER | completion evaluation PASS + its recommendations disposed |
| GATE:HYPOTHESIS / GATE:PLAN / AUDIT / GATE:QUALITY (PASS, a recommendation disposed `fix`) → owning phase → that gate's re-score | the orchestrator disposes of every recommendation as `fix` / `reject` + reason / `outside` / `operator` on the two scope-judgment questions — not on whether its file is already in the diff — in one ledger entry (`none` when there is none); an `operator` line is closed by a final-disposition line once the operator answers, and a resume re-enters past the gate only once all of this is closed. A `fix` travels the route its class names (`scripts/gate/remedy-route.sh route`; at GATE:HYPOTHESIS, the DIAGNOSE step that wrote the analysis; at GATE:PLAN, the DISPATCH spawn prompt for an item below the decision layer, an ARCHITECT re-discussion for a decision) and ends in that gate's re-entry re-score over the items the fix touched — except a GATE:PLAN item carried into DISPATCH, scored with the work it enters, and a target comment's direct commit, which keeps its no-re-score rule. One fix pass per gate per cycle — a directly related recommendation the re-score raises goes to the operator, and the bound is never a separation reason; the pass is not a FAIL and consumes no FAIL cap — a failing re-score is an ordinary FAIL, and a route through ARCHITECT consumes the ARCHITECT re-entry counter (`docs/autoflow-guide.md` > GATE:QUALITY > *Recommendation disposition*) |
