...
*Acceptance-criterion change*). When ARCHITECT returns `AC_CHANGE`, the operator's
...
| ARCHITECT → GATE:PLAN | feature design + verification design agreed, and the Reconcile check found no unauthorized acceptance-criterion change — a reduced verification disposition carrying a stated reason is not one: it passes the three-tier guard (deliberation → external reviewer at HANDOFF → operator only for AC content changes) |
