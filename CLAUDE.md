...
The same file carries the two Workflow facilitations' per-site values (`workflow_sites`) and the
ARCHITECT deliberation's turn ceilings (`deliberation_caps`), which the deliberation scripts load
at run time — no `model:` literal and no turn-ceiling literal remains in
...
Why the single mode: a named team spawn's final turn text is discarded by the runtime, so its report reached the orchestrator only through a mailbox turn that could silently drop it (#40 measured that loss — the record is retained at `docs/teammate-common-rules.md` > Result delivery path by spawn mode). A direct spawn's return value *is* its report, so the loss path does not exist. ADR-0021 measured the migration's remaining open question — whether the direct channel detects as well as the named one on VERIFY steps 3 and 4 — and returned `EQUAL_OR_BETTER`.
...
- The team-creation call, the `team_name` + `name` spawn form and `SendMessage` are no longer used: ADR-0017 recorded the removal, ADR-0021 discharged the blocking pilot, and this cycle carries the migration. A future multi-repo topology re-opens the cross-service coordination question through a new ADR rather than by reviving the channel.
...
- **Termination** (per [`docs/design-rationale.md`](docs/design-rationale.md) > Decision 7; turn model per issue #152): ARCHITECT convergence is **turn-based** — the Test AI and the Developer AI alternate single-participant turns, each reporting two facts (`modified`: did this turn edit a design document; `accept`: does this participant accept the current design), and the deliberation terminates **only** when two consecutive turns both report `modified: false, accept: true`. A turn that modified a document never converges, whatever its `accept` says — the modified design still owes the other side a review — and every other state simply continues to the next participant; no verdict history, document hash, revision pin or separate convergence-verification step is involved. The turn ceilings (cold and scope-bounded) are **config values** — `.claude/autoflow/spawn-policy.json` > `deliberation_caps`, never a code literal — and reaching the ceiling without convergence returns `ESCALATE`. VERIFY is a **single** self-check round (deterministic `next_action`, no internal loop). A facilitated deliberation never loops without a termination condition. An **`ESCALATE`d ARCHITECT deliberation may be resumed** by operator decision (`args: { issue: "N", resume: "true" }`), which re-enters it from its persisted register — Draft skipped, no document re-authored — and admits exactly one further exchange (two turns) per resume, unbounded in how many times it may be taken; the resumed run's first turn has no predecessor to pair with, so a resume converges only on its own two turns. The termination condition stays in code: each invocation computes its own ceiling, so control returns to the human on every extension rather than at a machine cap. See [`docs/design-rationale.md`](docs/design-rationale.md) > Decision 7 > *Termination under an ARCHITECT resume*, and the operator procedure in [`docs/autoflow-guide.md`](docs/autoflow-guide.md) > ARCHITECT.
...
- **[DENY]** **mailbox-delivery instruction in a spawn prompt**: every role is an anonymous direct spawn whose return value is its report, so a prompt must never instruct a spawn to deliver its result by message to the orchestrator. A spawn prompt states the delivery action as "return … as your final message". This is the inverse of the pre-migration hazard and rests on the same measurement: under the named channel the runtime discarded a spawn's final turn text, so the "final message" idiom mis-applied to a mailbox spawn induced the very loss it was meant to prevent (#40: 2 occurrences of the anonymous-direct idiom mis-applied to a mailbox spawn). The migration removed the channel that made those two idioms divergent — there is now one delivery path, and it is the return value. What happens to a spawn's final text is `docs/teammate-common-rules.md` > Result delivery path by spawn mode; the pilot that cleared the migration is ADR-0021.
