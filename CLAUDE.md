...
REFINE and raises in its report; or a gate's PASS recommendation whose fix needs one
...
| DIAGNOSE (structure eval) → DIAGNOSE (cause) | GATE:HYPOTHESIS structure PASS (code change required) + its `Medium`+ recommendations handled or carried to the gate that scores their fix |
...
| GATE:HYPOTHESIS → ARCHITECT | cause analysis PASS + code change required + its `Medium`+ recommendations handled or carried to the gate that scores their fix |
...
| GATE:PLAN → DISPATCH | plan evaluation PASS + its `Medium`+ recommendations handled or carried to the gate that scores their fix |
