...
**Two execution means, by the row's layer** (ADR-0024 D1/D2; issue #228). A `standing` row's test lives in the target's test tree, so it runs **through the target's declared test command**, discovered in this order, first hit wins: `.claude/autoflow.local.json` > `tests.command`, then the target's `CLAUDE.md` > Development Commands `Test` entry (ADR-0024 D3; a `tests.command` that is absent, `null` or empty is not a hit — the scaffold ships the key as `null` beside `tests.suite_plane: false`, issue #229); AutoFlow invokes it as declared and adjudicates nothing beyond its exit status. A **cycle-layer asset** — a default `automated` row's test, a `delivery-check`, a `manual` checklist — lives under `.autoflow/issue-{N}-local/`, outside that tree; it is authored as an executable file there and **invoked directly by its path**, never through the declared command and never through a driver AutoFlow ships. The recorded command **names the asset's path under the prefix**, which is what makes the run set's enumeration checkable: the verification design's table declares the set, and each row's recorded line is the witness that its asset executed — a row with no such line is `not-run`, never `passed`. A cycle that wants one command for several assets writes that driver *as* a cycle-layer asset under the same prefix. Such an asset is never committed, and is archived with the issue's other artifacts (ADR-0024 D2).
