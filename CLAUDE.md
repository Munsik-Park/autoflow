...
| GATE:HYPOTHESIS / GATE:PLAN / AUDIT / GATE:QUALITY (PASS, a recommendation attempt) → doc commit / RED / GREEN / ARCHITECT / DIAGNOSE → that gate's re-score, or → user | the PASS report's recommendation triage opens an attempt → the transition out of the gate waits until no attempt is open; a pause criterion or the attempt cap (Regressions below) → user (`active:false`, `phase:"awaiting-user"`). Which recommendation opens an attempt, its route, the pause criteria, the record and the cap's count are stated only in `docs/autoflow-guide.md` > GATE:QUALITY > *Recommendation triage* |
