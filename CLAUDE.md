...
| GATE:PLAN → DISPATCH | plan evaluation PASS + its recommendations triaged (no `Medium`+ open — every `Medium`+ re-enters ARCHITECT on a brief and GATE:PLAN re-scores; a `Low` below the decision layer is handed to the DISPATCH spawn prompt) |
