...
`scripts/audit/security-checklist.sh status` reports the change at AUDIT entry, the orchestrator
...
AUDIT           : Security Audit    — independent Evaluation AI (5 items × 10 points), the target's own security checklist at the version `scripts/audit/security-checklist.sh` names (none declared → the rubric alone)
...
| AUDIT | [`docs/autoflow-guide.md`](docs/autoflow-guide.md) > AUDIT; checklist: the target's own, declared at `.claude/autoflow.local.json` > `audit.security_checklist` and resolved by `scripts/audit/security-checklist.sh` |
