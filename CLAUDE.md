...
| PREFLIGHT → user | a fail-closed stop condition fails — bundle drift, reviewer-backend CLI absent, or a target-declared local check (`scripts/preflight/local-checks.sh`, issue #181) that does not pass after its declared repair → report and stop; DIAGNOSE does not begin. Absent declaration is a no-op recorded in the ledger, not a stop |
