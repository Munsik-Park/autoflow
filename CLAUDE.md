...
- The ledger is outside every teammate's scope — it is host-owned, written only by the orchestrator and its facilitator delegate: the facilitator appends entries during deliberation; the orchestrator appends each gate's verdict after the gate, records a loop-check observation (complaint class, witness, prior-change shape, cycle) on **every** review-response DIAGNOSE entry to seed the next cycle's baseline, appends the VERIFY detection record (the steps 3/4 outcomes, `verify-detection`-marked — a record, not a decision; see [`docs/autoflow-guide.md`](docs/autoflow-guide.md) > VERIFY > *Detection record*) at VERIFY exit, and — when a match pauses for the user — appends the user's re-entry decision as a separate entry after they answer.
