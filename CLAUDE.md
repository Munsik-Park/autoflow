...
- **Teammate message rules** (named spawns, for as long as the named mode remains — issue #136): every message to a named teammate re-writes its whole context at the first turn after a cold cache, and a wake message's attachments can re-write the prefix even seconds after the previous turn, so a message is a cost event, not a free signal. (a) **[DENY]** an ACK-only message (`"RED ACCEPTED; stand by"`, `"noted"`) — acceptance is recorded in the ledger / task state, not sent to the teammate; (b) never send a work instruction and then a separate `HOLD`, and send `GO` in the same message as the resuming instruction — `HOLD` is the standalone freeze message sent before a capture point and carries no work instruction; the order is fixed by `docs/teammate-common-rules.md` > Tree Quiesce and `docs/autoflow-guide.md` > VERIFY > Capture point, and this rule only removes the extra messages around it; (c) a teammate whose phase work is complete and that no later phase re-enters in this cycle is **shut down at that boundary, not held** — a HOLD-ed teammate re-writes its whole context at `GO` anyway and generates idle notifications in between, so holding buys nothing the re-spawn does not. The VERIFY → REFINE respawn above is one instance; DELIVER's teammate shutdown is another. Measurement and aggregation method: `docs/adr/0017-teammate-removal-feasibility.md` > Notes > C8.
...
- **[MUST]** The orchestrator delegates a multi-teammate deliberation to a **facilitator** realized as an isolated **`Workflow`** (Claude Code v2.1.154+), **not** as a nested Agent Team — a spawned teammate cannot create its own team and a team's lead is fixed for its lifetime, so a nested-team facilitator is not executable; and a peer-teammate facilitator is rejected on the ground recorded at [`docs/design-rationale.md`](docs/design-rationale.md) > Decision 8, which cites the measurement in `tests/manual/issue-52-manual-scenarios.md` > M1. The workflow runs the Developer-AI and Test-AI sub-agents in-script ("intermediate results stay in script variables instead of landing in Claude's context"), drives convergence under the Discussion Protocol, writes the converged artifacts to `.autoflow/*`, appends settled decisions to the decision ledger, and returns to the orchestrator **only** one structured result (per-phase — see [`docs/teammate-contracts.md`](docs/teammate-contracts.md) > Facilitator > Return Contract): ARCHITECT returns `{ verdict: CONVERGED|AC_CHANGE|ESCALATE, artifact paths, summary }` — `AC_CHANGE` pauses for the operator on an acceptance-criterion change and does not reach GATE:PLAN; VERIFY returns `{ test/impl self-check, next_action: RED|GREEN|SEQUENTIAL_FIX|EVALUATION_AI }`. The orchestrator never receives the round-by-round messages or the duplicate dual reports. Reference scripts: `.claude/workflows/{architect-deliberation,verify-cause-branch}.js`.
...
- **Termination** (per [`docs/design-rationale.md`](docs/design-rationale.md) > Decision 7): the facilitated discussion carries an explicit cap — ARCHITECT max **6 rounds** (a round = one Developer-AI ↔ Test-AI exchange cycle) → `ESCALATE` on no mutual ACCEPT, the cap round closed by a **closing half-round** (one Test-AI evaluation of the cap round's Developer-AI revision when that revision is a grounded ACCEPT — the second half of the sixth exchange, not a seventh round, so the cap and the round definition are unchanged); VERIFY is a **single** self-check round (deterministic `next_action`, no internal loop). A facilitated deliberation never loops without a termination condition. An **`ESCALATE`d ARCHITECT deliberation may be resumed** by operator decision (`args: { issue: "N", resume: "true" }`), which re-enters it from its persisted register — Draft skipped, no document re-authored — and raises the round cap by exactly one per resume (6 → 7 → 8 → …), unbounded in how many times it may be taken. The termination condition is unchanged and stays in code: each invocation computes its own ceiling and admits exactly one further round, so control returns to the human on every round rather than at a machine cap. See [`docs/design-rationale.md`](docs/design-rationale.md) > Decision 7 > *Termination under an ARCHITECT resume*, and the operator procedure in [`docs/autoflow-guide.md`](docs/autoflow-guide.md) > ARCHITECT.
...
**Acceptance-criterion decisions** (`[ac-decision]`). Changing an issue's acceptance criteria is the
**operator's** authority, never a deliberation's. When ARCHITECT returns `AC_CHANGE`, the operator's
...
Reconcile channel and both gate backstops key on the marker; `LEDGER_SEED_RULE` keys on the
authority value, so a resumed deliberation treats the operator's call as settled rather than
re-arguing it. `ARCHITECT ac-change` — the authority a paused run's own outcome entry carries — is
deliberately outside that settled set: it records content still awaiting the decision. When the
...
**Regressions** (cap semantics: "max N×" = N regressions permitted; the gate escalates to a human on the **(N+1)th** FAIL — e.g. `max 2×` → escalate on the 3rd FAIL): GATE:HYPOTHESIS cause FAIL → DIAGNOSE (max 2×). GATE:PLAN FAIL → ARCHITECT (max 3×; a VERIFY design contradiction re-deliberation consumes this same counter, so ARCHITECT re-entries are capped at 3 per cycle regardless of which phase triggered them; an ARCHITECT **resume** is not a re-entry — it re-enters an `ESCALATE`d deliberation from its persisted register as an operator-decided extension of the deliberation already counted, raising that deliberation's round cap by one and consuming no re-entry-counter budget, unbounded in how many times the operator may take it; the same holds for an acceptance-criterion pause — an AC pause and its resume consume no ARCHITECT re-entry budget, since the pause is a human authority checkpoint inside the deliberation already counted, not a new deliberation). VERIFY FAIL → cause-branched fix (max 3 round-trips). REFINE FAIL → Developer AI fixes and re-runs (max 2×; on second failure, abandon refactor and proceed to VALIDATE with the Green state). AUDIT FAIL → fix and re-evaluate (max 2×). GATE:QUALITY FAIL → class-routed re-entry (doc commit / RED / GREEN / ARCHITECT by `remedy_class`; max 3× — the cap counts FAILs, not the distance re-entered; a `design` route also consumes the ARCHITECT re-entry counter above). VALIDATE step-1 whole-tree sweep FAIL → RED or GREEN by class (no new cap — the existing GREEN ↔ VERIFY round-trip rules apply). INTEGRATE FAIL → GREEN (`impl`). HANDOFF failure → cause classification: code issue → RED; environment / push rejection → HANDOFF internal retry (max 2×). reviewer-review auto-resolution (Medium+ found at HANDOFF) → review-response (max 7×; on the 7th consecutive (per the count window in `docs/autoflow-guide.md` step 6.5) without the `blocked-by-review` label clearing, pause for the user).
...
- **[DENY]** **"final message" instruction in a named-spawn prompt**: a prompt that tells a **named team spawn** (`team_name` + role-prefixed `name` — Test AI, Developer AI) to return its result "as your final message" instructs the one delivery path the runtime discards, and directly induces the loss it is meant to prevent (#40: 2 occurrences of the anonymous-direct idiom mis-applied to a mailbox spawn). A named-spawn prompt states the delivery action as `SendMessage(to: "team-lead")`; the "return … as your final message" idiom belongs to anonymous direct spawns only. Which roles are named is fixed by [Spawn mode by role lifetime](#spawn-mode-by-role-lifetime); what happens to each mode's final text is `docs/teammate-common-rules.md` > Result delivery path by spawn mode.
- **Named-spawn non-delivery reading**: a delivered report leaves a `summary: "[to main] …"` field on that teammate's subsequent idle notification; a notification with no `summary` field reads as **report not sent**, not as "still working". Recover by (a) verifying the teammate's `.autoflow/*` artifact by shell, or (b) re-requesting the report with `SendMessage` — both were used in the #40 cycle; do not wait for a follow-up turn that will not come. Observed, not guaranteed: the signal matched delivery across all 12 subagents of the #40 cycle (`docs/teammate-common-rules.md` > Result delivery path by spawn mode) — re-derivable through `tests/manual/issue-42-manual-scenarios.md` M1; treat it as an observation to re-check, not a runtime guarantee. This is the named-spawn counterpart to the **Done** direct-spawn case in *Teammate idle handling* above.
