...
- The team-creation call, the `team_name` + `name` spawn form and `SendMessage` are no longer used: ADR-0017 recorded the removal, ADR-0019 discharged the blocking pilot, and this cycle carries the migration. A future multi-repo topology re-opens the cross-service coordination question through a new ADR rather than by reviving the channel.
...
- **Concurrent spawns**: keep ≤ 5 `Agent` spawns in flight at once. Each spawn is anonymous and direct, so there is no session occupancy to manage — the cap bounds concurrent token burn, not team membership.
...
- **Teammate idle handling**: continue work when (a) a spawn returns its report, (b) a Bash result you initiated returns, or (c) the user types a new prompt. A spawned subagent has no future turn to receive a background-completion notification, so when a **Done** direct-spawn produces no report, do not wait for a follow-up: verify its `.autoflow/*` artifact by shell and proceed (see "Incomplete output is never ground truth").
