...
The operator's decision on what a rule is for (issue #225; ADR-0024). This section is the single home of the four principles and of the local-verification rule below; every other document cites it rather than restating it.
...
**Local verification** (the instance of principle 2 the phases cite). A change runs, locally and once, the tests the working AI judges it requires — at minimum every `automated` row this cycle authored or changed and every `delivery-check` row (ADR-0024 M) — and records each run's command, the log its output was written to, and the summary line read from that log (Reporting Format item 5). Regression verification is HANDOFF's CI (step 5). There is no local whole-tree run: none scheduled, none held in reserve.
...
AutoFlow's own suite plane — the header contract, the selector and the runner — applies only where the target opted in (`.claude/autoflow.local.json` > `tests.suite_plane: true`) and in this repository, whose standing layer depends on it (ADR-0024 D5).

## Development Commands

- **Test**: `bash scripts/test/run-suites.sh` — the suites the change requires, selected from each suite's `# ci-subject:` header against the resolved delta (this repository opts into its own suite plane: `.claude/autoflow.local.json` > `tests`). The workflow scripts' regression is `node test/workflows/run.mjs` (CI job `workflow-regression`).
...
- The ledger is outside every role spawn's scope — it is host-owned, written only by the orchestrator and its facilitator delegate: the facilitator appends the deliberation's agreed conclusions under the authority `ARCHITECT agreed`; the orchestrator appends each gate's verdict after the gate, records a loop-check observation (complaint class, witness, prior-change shape, cycle) on **every** review-response attempt — at DIAGNOSE entry, or at HANDOFF step 6.5 for a thin route that runs no DIAGNOSE (#192) — and on every attempt to fix a gate's PASS recommendation, to seed the next comparison's baseline, appends the VERIFY detection record (the steps 3/4 outcomes, `verify-detection`-marked — a record, not a decision; see [`docs/autoflow-guide.md`](docs/autoflow-guide.md) > VERIFY > *Detection record*) at VERIFY exit, and — when a match pauses for the user — appends the user's re-entry decision as a separate entry after they answer.

**Entry identifier**. A settled-decision entry is headed `## <ID> — <title> (cycle <C>, <PHASE>)`, where `<ID>` is a one-letter **writer namespace** followed by a serial that counts within that namespace only. An auto-triggered review-response entry carries its HANDOFF marker in the same grammar: `## O<n> — <title> (cycle <C>, HANDOFF) [review-autofix]` — the marker stays at the end of the heading, so the cap's count predicate ([`docs/autoflow-guide.md`](docs/autoflow-guide.md) > HANDOFF step 6.5) is unaffected by the identifier prefix. An attempt to fix a gate's PASS recommendation carries the gate's marker in the same grammar, `## O<n> — <title> (cycle <C>, <GATE>) [gate-autofix]` ([`docs/autoflow-guide.md`](docs/autoflow-guide.md) > GATE:QUALITY > *PASS recommendations*); a separate marker keeps the `[review-autofix]` count to reviewer findings. Record entries (`verify-detection`, `preflight-local-checks`) are level-3 headings and carry no identifier.
...
be wrong, so a content change reaches the operator from each point where the work can show it: an
agreed conclusion of the ARCHITECT deliberation, presented before GATE:PLAN; a problem a role meets
during RED, GREEN, VERIFY or REFINE and raises in its report; or a gate's PASS recommendation whose fix needs one
([`docs/autoflow-guide.md`](docs/autoflow-guide.md) > ARCHITECT > *Report routing*, > GATE:QUALITY >
*PASS recommendations*). The change excludes, revises or splits a criterion, or adds one. The
...
| review-response loop check → user | the trigger comment repeats the immediately-prior review-response attempt's complaint class with a new witness case → reply on PR + await the user's re-entry decision (`phase: awaiting-user`). Runs at DIAGNOSE entry, at HANDOFF step 6.5 before a thin route (#192), and before each attempt to fix a gate's PASS recommendation, where the trigger is that recommendation and the reply goes on the PR only when one exists — one call per attempt, on every route |
| DIAGNOSE (structure eval) → DIAGNOSE (cause) | GATE:HYPOTHESIS structure PASS (code change required) + its `Medium`+ recommendations handled |
...
| GATE:HYPOTHESIS → ARCHITECT | cause analysis PASS + code change required + its `Medium`+ recommendations handled |
...
| AUDIT → GATE:QUALITY | security audit PASS + its `Medium`+ recommendations handled |
| GATE:QUALITY → DELIVER | completion evaluation PASS + its `Medium`+ recommendations handled |
| GATE:HYPOTHESIS / GATE:PLAN / AUDIT / GATE:QUALITY (PASS with a `Medium`+ recommendation) → fix → that gate's re-score / user | the recommendations take the procedure a reviewer finding takes (HANDOFF step 6.5), with the evaluator's classification in place of the ingesting subagent and the gate's re-score in place of the reviewer re-review: each `Medium`+ recommendation carries the evaluator's `remedy_class`, `scripts/gate/remedy-route.sh route` names the entry point — re-entered as a FAIL's is where the cycle has passed it, and entered as input where it lies ahead — and the gate whose report raised it re-scores a fix made behind it, while a fix made in a phase ahead is scored with the change by the gates that score that phase; step 6.5's four pause criteria and its 7-attempt cap apply, the cap counted over `[gate-autofix]` ledger entries; a `Low` one is the orchestrator's judgment and one that is not this issue's work goes to a separate issue; a re-score that FAILs is an ordinary FAIL of that gate (`docs/autoflow-guide.md` > GATE:QUALITY > *PASS recommendations*) |
