...
# The topic, stated once: the only per-run text, identical for every turn. The
# decision ledger's settled entries are part of the topic so a settled decision
# is read, not re-argued. The orchestrator's brief, when given at init, is
# carried verbatim as part of it.
topic_text() {   # <issue> [<brief>]
  local issue="$1" brief="${2:-}"
  printf 'Issue #%s. Inputs: .autoflow/issue-%s-phase-a.md (code structure), .autoflow/issue-%s-phase-b.md (issue analysis and the acceptance-criteria table), any other .autoflow/issue-%s-*.md, and the decision ledger .autoflow/issue-%s-ledger.md if it exists — an entry there under authority "ARCHITECT agreed", "ARCHITECT mutual ACCEPT", "ARCHITECT rejected" or "operator decision" is settled and is reopened only on a fact verified now that was unavailable when it was written. Question: what is the architecture decision layer of the feature design for this issue — the decisions, the constraints they hold under, and the alternatives rejected and why — and how is each acceptance criterion verified, with the failure mode each verification catches stated in the Failure mode column of the verification design, not in the feature design? And what is the scope of this cycle beyond the acceptance criteria — for each problem the confirmed cause carries (a Scope judgments section among the inputs, or one you find), is it directly related to this issue and desirable to fix in this cycle, judged under docs/submodule-common-rules.md > Change Surface Rules > Scope judgment? Settle decisions only: a change table of files, a per-suite disposition or an oracle condition clause is derived at RED/GREEN entry by the execution roles and is out of scope here (docs/phases/architect.md > Output artifacts). The test for a claim: if it were wrong, would the design have to be revisited, or would it just be fixed where it is found?' \
