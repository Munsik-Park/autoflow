...
if [[ -z "$BASE_REF" ]]; then
  ...
  allow_list=(
    ...
    # amendment lands on docs/teammate-contracts.md, docs/design-rationale.md
    # and docs/evaluation-system.md (maintained-docs.md, setup/manifest.json,
    # and tests/fixtures/doc-invariants.json are already admitted above),
    # plus the two new cycle-scoped RED suites.
    ...
    # its manual-scenario lane, plus the F5 source
    # (.claude/workflows/architect-deliberation.js — the composition-oracle
    # two-literal Test-AI prompt edit, feature design §3.4). The doc
