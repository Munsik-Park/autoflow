...
# =============================================================================
echo ""
echo "=== AC-62-31 (RED discriminator) — the two unconditional ok-count pin homes bumped to 58 in the same commit ==="
...
NEW_27_EQ="$(grep -c -- '-eq 58 \]"' "$CANON_27" || true)"
NEW_27_LABEL="$(grep -cF 'AC-27-20c: harness ok-line count == B14 (58)' "$CANON_27" || true)"
STALE_27_EQ="$(grep -c -- '-eq 43 \]"' "$CANON_27" || true)"
assert_true "AC-62-31a: test-issue-27's assertion pins -eq 58 exactly once (got: $NEW_27_EQ)" \
  "[ \"$NEW_27_EQ\" -eq 1 ]"
assert_true "AC-62-31a-label: the bumped '(58)' assertion label is present exactly once (got: $NEW_27_LABEL)" \
  "[ \"$NEW_27_LABEL\" -eq 1 ]"
assert_true "AC-62-31a-stale: no -eq 43 ok-count literal survives in test-issue-27 (got: $STALE_27_EQ)" \
  "[ \"$STALE_27_EQ\" -eq 0 ]"

NEW_59_EXPECTED="$(grep -cF 'EXPECTED_OK=58' "$CANON_59" || true)"
STALE_59_EXPECTED="$(grep -cF 'EXPECTED_OK=43' "$CANON_59" || true)"
assert_true "AC-62-31b: test-issue-59's EXPECTED_OK=58 appears exactly once (got: $NEW_59_EXPECTED)" \
  "[ \"$NEW_59_EXPECTED\" -eq 1 ]"
assert_true "AC-62-31c: no stale EXPECTED_OK=43 survives in test-issue-59 (got: $STALE_59_EXPECTED)" \
