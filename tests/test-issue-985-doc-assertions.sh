...
# AC1-INTERNAL-DOCS-ABSENT, AC1-NO-DANGLING-REF,
...
assert_true "AC1-BACKLOG-EMPTY-START: docs/improvement-backlog.md is tracked with no internal finding blocks" \
  "git ls-files --error-unmatch 'docs/improvement-backlog.md' >/dev/null 2>&1 && ! grep -qE '^### \`([a-zA-Z0-9_-]+-[0-9])' '$PROJECT_ROOT/docs/improvement-backlog.md' 2>/dev/null"
